...
function trigger_webhooks($usuario_id, $event_data, $trigger_event, $produto_id = null) {
    ...
    foreach ($webhooks as $url) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            'X-CheckoutPRO-Event: ' . $trigger_event, // Cabeçalho personalizado
            'User-Agent: FlixMembersWebhookAgent/1.0' // User-Agent personalizado
        ]);
...
function trigger_utmfy_integrations($usuario_id, $event_data, $trigger_event, $produto_id = null, $mercado_pago_payment_data = []) {
    ...
    // Mapeamento de status da CheckoutPRO para status da UTMfy
    $status_map = [
        'approved'     => 'paid',
    ...
    // Mapeamento de métodos de pagamento da CheckoutPRO para UTMfy
    $payment_method_map = [
        'Pix' => 'pix',
        'Boleto' => 'boleto',
    ...
    foreach ($integrations as $api_token) {
        ...
        foreach ($event_data['produtos_comprados'] as $product) {
            ...
            $products_payload[] = [
                ...
                'planId' => null, // Assumindo que CheckoutPRO não tem 'planId'
                'planName' => null, // Assumindo que CheckoutPRO não tem 'planName'
        ...
        if (empty($created_at_utc)) {
            ...
            log_webhook("UTMFY: data_venda da CheckoutPRO vazia, usando tempo atual UTC para createdAt.");
        }


        $payload = [
            'orderId' => (string)$event_data['transacao_id'],
            'platform' => 'FlixMembersBR6', // Nome da sua plataforma
            ...
            'commission' => [
                ...
                'gatewayFeeInCents' => 0, // Ajustar se a CheckoutPRO tiver uma taxa de gateway fixa/conhecida
    ...
    switch ($delivery_type) {
        ...
        case 'arquivo':
            if (!empty($delivery_content)) {
                ...
                $pdf_abs_path = __DIR__ . '/../uploads/' . $safe_content;
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ($action === 'get_live_notifications') {
                $stmt = $pdo->prepare("
                    SELECT 
                        n.id, n.tipo, n.mensagem, n.valor, n.metodo_pagamento, n.venda_id_fk, p.nome as produto_nome, p.foto as produto_foto 
                    FROM notificacoes n
                ...
                // Fetch metodo_pagamento from vendas table if available
                foreach ($live_notifications as &$notification) {
                    if (!empty($notification['venda_id_fk'])) {
                        $stmt_method = $pdo->prepare("SELECT metodo_pagamento FROM vendas WHERE id = ? LIMIT 1");
                        $stmt_method->execute([$notification['venda_id_fk']]);
                        $notification['metodo_pagamento'] = $stmt_method->fetchColumn() ?: 'Desconhecido';
                    } else {
                        $notification['metodo_pagamento'] = $notification['metodo_pagamento'] ?? 'Desconhecido';
                    }
                }
                unset($notification);
