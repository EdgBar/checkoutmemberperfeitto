...
try {
    ...
    if (!in_array($event_type, $allowed_event_types)) {
        http_response_code(400);
        error_log("FLIXMEMBERS_TRACK_ENDPOINT: Tipo de evento inválido: '$event_type'. Dados recebidos: " . print_r($data, true));
    ...
    if (!$tracking_product_id_db) {
        ...
        error_log("FLIXMEMBERS_TRACK_ENDPOINT: Tracking ID inválido ou não encontrado: '$tracking_id'. Dados recebidos: " . print_r($data, true));
    ...
    if ($event_type === 'page_view' || $event_type === 'initiate_checkout') {
        error_log("FLIXMEMBERS_TRACK_ENDPOINT: Verificando evento existente para '$event_type' na sessão '$session_id' para tracking_product_id '$tracking_product_id_db'.");
        ...
        if ($check_existing_stmt->fetchColumn() > 0) {
            error_log("FLIXMEMBERS_TRACK_ENDPOINT: Evento '$event_type' já registrado para a sessão '$session_id' e tracking_product_id '$tracking_product_id_db'. Ignorando inserção.");
    ...
    if ($insert_event) {
        ...
        if ($stmt_insert->execute()) {
            error_log("FLIXMEMBERS_TRACK_ENDPOINT: Evento '$event_type' registrado com sucesso. Session ID: '$session_id', Tracking Product DB ID: '$tracking_product_id_db'.");
