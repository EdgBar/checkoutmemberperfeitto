...
$notification_helper = __DIR__ . '/../helpers/notification_helper.php';
if (file_exists($notification_helper)) {
...
function process_approved_payment_dispatches($pdo, $payment_id, $gateway_name = '') {
    try {
        ...
        // 5. Criar notificação de venda aprovada (usando helper)
        try {
            $venda_id = $main_sale['id'] ?? null;
            $produto_nome = $main_sale['produto_nome'] ?? 'Produto';
            
            if (function_exists('notificar_compra_aprovada')) {
                $notif_criada = notificar_compra_aprovada($pdo, $venda_id, $valor_total_compra, $produto_nome);
                if ($notif_criada) {
                    log_check_status("$gateway_name: Notificação 'Compra Aprovada' criada com sucesso para venda_id: $venda_id");
                } else {
                    log_check_status("$gateway_name: Notificação já existia ou erro para venda_id: $venda_id");
                }
            } else {
                // Fallback se helper não existir
                $usuario_id = $main_sale['usuario_id'];
                $metodo = $main_sale['metodo_pagamento'] ?? 'Não informado';
                
                // Primeiro, deletar notificações pendentes (Pix Gerado, Boleto Gerado)
                $pdo->prepare("DELETE FROM notificacoes WHERE venda_id_fk = ? AND tipo IN ('Pix Gerado', 'Boleto Gerado')")->execute([$venda_id]);
                
                $stmt_check = $pdo->prepare("SELECT id FROM notificacoes WHERE venda_id_fk = ? AND tipo = 'Compra Aprovada' LIMIT 1");
                $stmt_check->execute([$venda_id]);
                
                if (!$stmt_check->fetch()) {
                    $mensagem = "Nova venda: {$produto_nome} - R$ " . number_format($valor_total_compra, 2, ',', '.');
                    $stmt_notif = $pdo->prepare("INSERT INTO notificacoes (usuario_id, tipo, mensagem, valor, metodo_pagamento, venda_id_fk, link_acao, data_notificacao) VALUES (?, 'Compra Aprovada', ?, ?, ?, ?, ?, NOW())");
                    $stmt_notif->execute([$usuario_id, $mensagem, $valor_total_compra, $metodo, $venda_id, "/index?pagina=vendas&id={$venda_id}"]);
                    log_check_status("$gateway_name: Notificação criada (fallback) para venda_id: $venda_id");
                }
            }
        } catch (Exception $e) {
            log_check_status("$gateway_name: Erro ao criar notificação: " . $e->getMessage());
        }
