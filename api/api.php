...
function require_csrf_for_modifying_actions($csrf_token = null) {
    ...
    if ($csrf_token === null) {
        if (isset($_POST['csrf_token'])) {
            ...
            // Usar input global já lido (não consumir php://input novamente)
            $content_type = $_SERVER['CONTENT_TYPE'] ?? '';
            if (strpos($content_type, 'application/json') !== false) {
                $input = $GLOBALS['parsed_input'] ?? null;
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    switch ($delivery_type) {
        ...
        case 'arquivo':
            if (!empty($delivery_content)) {
                // Caminho absoluto robusto do uploads/ (evita falha por cwd diferente)
                $base_dir = __DIR__;
                $parent_dir = realpath($base_dir . DIRECTORY_SEPARATOR . '..');
                $project_root = ($parent_dir && is_dir($parent_dir . DIRECTORY_SEPARATOR . 'uploads')) ? $parent_dir : $base_dir;
                $uploads_dir = rtrim($project_root, "\\/") . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;

                $safe_content = basename((string)$delivery_content);
                $pdf_abs_path = $uploads_dir . $safe_content;
                if (!empty($safe_content) && file_exists($pdf_abs_path) && is_readable($pdf_abs_path)) {
                    return ['success' => true, 'product_name' => $product_name, 'content_type' => 'pdf', 'content_value' => $pdf_abs_path];
                } else {
                    return ['success' => false, 'message' => "Arquivo PDF não encontrado ou ilegível."];
                }
            } else {
        ...
        case 'produto_fisico':
        case 'manual':
            return ['success' => true, 'product_name' => $product_name, 'content_type' => 'produto_fisico', 'content_value' => null];
        default:
...
function send_delivery_email_consolidated($to_email, $customer_name, $processed_products_for_email, $member_area_password, $member_area_login_url, $email_subject, $email_html_template, $address_data = null, $setup_token = null) {
    ...
    try {
        ...
        $smtp_config = [
            ...
            'from_name' => $smtp_configs_raw['smtp_from_name'] ?? 'CheckoutPRO'
        ];
        ...
        if (empty(trim($email_html_template))) {
            ...
            $cor_primaria = '#32e768';
            $nome_plataforma = 'CheckoutPRO';
            
            if (function_exists('getSystemSetting')) {
                $cor_primaria = getSystemSetting('cor_primaria', '#32e768');
                $nome_plataforma = getSystemSetting('nome_plataforma', 'CheckoutPRO');
            } else {
                ...
                $nome_plataforma = $sistema_configs['nome_plataforma'] ?? 'CheckoutPRO';
            }
...
try {
    ...
    // Para POST requests, ler o input UMA VEZ e armazenar globalmente
    $GLOBALS['raw_input'] = null;
    $GLOBALS['parsed_input'] = null;
    
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $GLOBALS['raw_input'] = file_get_contents('php://input');
        $GLOBALS['parsed_input'] = json_decode($GLOBALS['raw_input'], true);
        
        // Se não tiver na query string, tenta do corpo
        if (empty($action) && isset($GLOBALS['parsed_input']['action'])) {
            $action = $GLOBALS['parsed_input']['action'];
        }
    }
    ...
    // NOVO: Ação para obter dados para a Jornada CheckoutPRO
    if ($action == 'get_jornada_starfy_data') {
        ...
        try {
            ...
            ob_clean();
            error_log("API: Erro ao buscar dados da Jornada CheckoutPRO (get_jornada_starfy_data): " . $e->getMessage());
            echo json_encode(['error' => 'Erro ao buscar dados da Jornada CheckoutPRO.']);
            exit;
        }
    }
        ...
        try {
            ...
            $sql_offers = "
                SELECT
                    ...
                    peo.saber_mais_url AS saber_mais_url
                FROM
            ...
            foreach ($offers as &$o) {
                if (!array_key_exists('saber_mais_url', $o)) {
                    $o['saber_mais_url'] = null;
                }
            }
            unset($o);
            ...
            if (strpos($e->getMessage(), 'saber_mais_url') !== false) {
                $sql_offers_fallback = "
                    SELECT
                        p_offer.id AS product_id,
                        p_offer.nome AS product_name,
                        ...
                        p_offer.checkout_hash,
                        u.nome AS infoprod_name
                    FROM product_exclusive_offers peo
                    JOIN produtos p_source ON peo.source_product_id = p_source.id
                    JOIN produtos p_offer ON peo.offer_product_id = p_offer.id
                    JOIN usuarios u ON p_offer.usuario_id = u.id
                    WHERE peo.is_active = 1
                    AND peo.source_product_id IN ({$owned_product_ids_placeholder})
                    AND p_offer.tipo_entrega = 'area_membros'
                    ...
                    LIMIT 10
                ";
                $stmt_f = $pdo->prepare($sql_offers_fallback);
                $param_index = 1;
                foreach ($owned_product_ids as $id) {
                    $stmt_f->bindValue($param_index++, $id, PDO::PARAM_INT);
                }
                foreach ($owned_product_ids as $id) {
                    $stmt_f->bindValue($param_index++, $id, PDO::PARAM_INT);
                }
                $stmt_f->execute();
                $offers = $stmt_f->fetchAll(PDO::FETCH_ASSOC);
                foreach ($offers as &$o) {
                    $o['saber_mais_url'] = null;
                }
                unset($o);
    ...
    if ($action == 'generate_tracking_script') {
        ...
        // Usar caminhos fixos para garantir que funcionem em qualquer contexto (Page Builder, etc)
        $track_beacon_endpoint = $protocol . $domainName . '/api/track_beacon.php';
        $track_event_endpoint = $protocol . $domainName . '/api/api.php';
...
<script>
    (function() {
        ...
        console.log('CheckoutPRO Track Script Loaded. Tracking ID:', STARFY_TRACK_ID, 'Checkout Base URL:', CHECKOUT_BASE_URL);
        ...
        function sendEvent(eventType, eventData = {}) {
            ...
            console.log('CheckoutPRO Track: Sending event', eventType, 'with payload:', payload);
            ...
            fetch(TRACK_EVENT_ENDPOINT, {
                ...
                if (!response.ok) {
                    console.error('CheckoutPRO Track: Erro ao enviar evento ' + eventType + ':', response.statusText);
                } else {
                    console.log('CheckoutPRO Track: Evento ' + eventType + ' enviado com sucesso.');
                }
            }).catch(error => {
                console.error('CheckoutPRO Track: Erro de rede ao enviar evento ' + eventType + ':', error);
            });
        }
        ...
        function trackPageViewBeacon() {
            ...
            // Log para debug
            console.log('CheckoutPRO Track: Sending Page View via Beacon to:', img.src);
            ...
            img.onerror = () => { console.error('CheckoutPRO Track: Erro ao carregar beacon de rastreamento.'); if(img.parentNode) img.parentNode.removeChild(img); };
        }

        // --- INICIALIZAÇÃO ---

        // 1. DISPARO INICIAL (Page View ou Checkout)
        function initTracking() {
            // Verifica se está na página de checkout
            const isCheckoutPage = window.location.href.includes('/checkout') || window.location.href.includes('checkout?p=');
            
            if (isCheckoutPage) {
                // Se está no checkout, registra como initiate_checkout
                const checkoutHash = new URLSearchParams(window.location.search).get('p') || 'direct_checkout';
                sendEvent('initiate_checkout', { checkout_hash: checkoutHash, via: 'direct_access' });
                console.log('CheckoutPRO Track: Checkout detectado! Registrando initiate_checkout para hash:', checkoutHash);
            } else {
                // Se está em outra página (funil), registra page_view
                trackPageViewBeacon();
            }
        }
        ...
        // 3. MONITORAMENTO DE INTERAÇÕES (Início de Checkout via clique)
            ...
            while (target && target !== document.body) {
                ...
                if (target.tagName === 'A' && target.href && target.href.includes(CHECKOUT_ENDPOINT_PARTIAL)) {
                    ...
                    if (checkoutHash) {
                        ...
                        console.log('CheckoutPRO Track: Evento initiate_checkout disparado para hash:', checkoutHash, 'no link A.');
                ...
                if (target.classList && target.classList.contains('starfy-checkout-btn')) {
                    ...
                    console.log('CheckoutPRO Track: Evento initiate_checkout disparado via classe para hash:', checkoutHash);
                    return; // Evento capturado, sair
                }
        ...
        document.addEventListener('submit', (e) => {
            ...
            if (form.tagName === 'FORM' && form.action.includes(CHECKOUT_ENDPOINT_PARTIAL)) {
                ...
                if (checkoutHash) {
                    ...
                    console.log('CheckoutPRO Track: Evento initiate_checkout disparado via FORM submit para hash:', checkoutHash);
...
EOT;
    ...
    if ($action == 'get_starfy_tracking_data') {
        ...
        // 3. Total Purchases (conta compras que tiveram algum evento de tracking - page_view ou initiate_checkout)
        // Só conta compras de usuários que passaram pelo funil rastreado
        try {
            // Primeiro verifica se há eventos de tracking para este produto
            $sql_check_events = "SELECT COUNT(*) FROM starfy_tracking_events WHERE tracking_product_id = :tracking_product_id";
            $stmt_check = $pdo->prepare($sql_check_events);
            $stmt_check->bindParam(':tracking_product_id', $tracking_product_id, PDO::PARAM_INT);
            $stmt_check->execute();
            $has_tracking_events = (int)$stmt_check->fetchColumn() > 0;
            
            if ($has_tracking_events) {
                // Se há eventos de tracking, conta apenas compras de sessões rastreadas
                $sql_purchases = "
                    ...
                    AND v.checkout_session_uuid IN (
                        ...
                        WHERE ste.tracking_product_id = :tracking_product_id 
                        {$date_filter_sql_ste}
                    )
                ";
                ...
                // Se não há eventos de tracking ainda, mostra 0
                $purchases = 0;
            }
            error_log("API: Purchases - Has tracking: " . ($has_tracking_events ? 'SIM' : 'NÃO') . ", Original Product ID: $original_product_id, Result: $purchases");
        } catch (PDOException $e) {
            error_log("API: Erro PDO na consulta de purchases: " . $e->getMessage());
            throw $e;
        }

        // 4. Sales details (apenas vendas de sessões rastreadas)
        try {
            if ($has_tracking_events) {
                $sql_sales_details = "
                    SELECT 
            ...
            error_log("API: Sales Details - Has tracking: " . ($has_tracking_events ? 'SIM' : 'NÃO') . ", Original Product ID: $original_product_id");
        } catch (PDOException $e) {
            error_log("API: Erro PDO na consulta de sales_details: " . $e->getMessage());
            throw $e;
        }
    ...
    if ($action == 'test_webhook' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $test_payload = [
            ...
            'data' => [
                'test_message' => 'Este é um evento de teste da CheckoutPRO BR 3',
                ...
                'comprador_telefone' => '558195511599',
    ...
    if ($action == 'clone_url' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Usar input global já lido no início da API
        $input = $GLOBALS['parsed_input'] ?? [];
    ...
    if ($action == 'save_cloned_site' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Validar CSRF com token do input
