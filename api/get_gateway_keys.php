...
try {
    ...
    // Buscar chaves públicas do vendedor (stripe_publishable_key pode não existir em DB antigo)
    try {
        $stmt_vendedor = $pdo->prepare("SELECT mp_public_key, beehive_public_key, hypercash_public_key, efi_payee_code, stripe_publishable_key FROM usuarios WHERE id = ? LIMIT 1");
        $stmt_vendedor->execute([$produto['usuario_id']]);
        $vendedor_data = $stmt_vendedor->fetch(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        if (strpos($e->getMessage(), 'stripe_publishable_key') !== false) {
            ...
            $stmt_vendedor->execute([$produto['usuario_id']]);
            $vendedor_data = $stmt_vendedor->fetch(PDO::FETCH_ASSOC);
            if ($vendedor_data) $vendedor_data['stripe_publishable_key'] = '';
        } else {
            throw $e;
        }
    }
