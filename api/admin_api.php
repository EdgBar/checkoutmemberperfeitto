...
try {
    ...
    function getSmtpConfigFromRequest($pdo, $input_data) {
        $smtp_config = [
            ...
            'from_name' => $input_data['smtp_from_name'] ?? 'CheckoutPRO',
        ];
    ...
    elseif ($action == 'get_users') {
        ...
        $empty_response = function() use ($page) {
        ...
        try {
            ...
            // Selecionar apenas colunas essenciais (telefone pode não existir em algumas instalações)
            $sql = "SELECT u.id, u.usuario, u.nome, u.tipo FROM usuarios u {$where_clause} ORDER BY u.id DESC LIMIT " . (int)$limit . " OFFSET " . (int)$offset;
            ...
            foreach ($users as &$u) {
                if (!array_key_exists('telefone', $u)) {
                    $u['telefone'] = '';
                }
            }
            unset($u);
        } catch (PDOException $e) {
            error_log("ADMIN_API get_users: " . $e->getMessage());
            $empty_response();
        }
    ...
    elseif ($action == 'get_user_details') {
        ...
        // Em algumas instalações antigas a coluna `telefone` pode não existir.
        // Tentamos buscar com `telefone`, e se falhar fazemos fallback sem ela.
        try {
            ...
            $stmt = $pdo->prepare("SELECT id, usuario, nome, tipo FROM usuarios WHERE id = :id");
            $stmt->bindParam(':id', $user_id, PDO::PARAM_INT);
            $stmt->execute();
            $user = $stmt->fetch(PDO::FETCH_ASSOC);
            if (is_array($user)) {
        ...
        // Para clientes, enriquecer telefone com o da última venda se o da tabela estiver vazio
        if ($user && (empty(trim($user['telefone'] ?? '')))) {
            try {
                $stmt_tel = $pdo->prepare("SELECT comprador_telefone FROM vendas WHERE comprador_email = ? AND comprador_telefone IS NOT NULL AND comprador_telefone != '' ORDER BY data_venda DESC LIMIT 1");
                $stmt_tel->execute([$user['usuario']]);
                $row = $stmt_tel->fetch(PDO::FETCH_ASSOC);
                if ($row && !empty(trim($row['comprador_telefone']))) {
                    $user['telefone'] = $row['comprador_telefone'];
                }
            } catch (PDOException $e) {
                            ...
                            if ($plano) {
                                ...
                                if ($has_data_vencimento) {
                                    ...
                                    $stmt_assinatura = $pdo->prepare("
                                        ...
                                        VALUES (?, ?, 'ativa', ?)
                                    ");
                                    $stmt_assinatura->execute([$new_user_id, $plano_id, $data_inicio]);
                                }
    ...
    elseif ($action == 'update_user' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        error_log("ADMIN_API: (update_user) tipo do usuário: " . var_export($tipo, true));
        error_log("ADMIN_API: (update_user) user_id: " . var_export($user_id, true));
        ...
        if ($stmt->execute($params)) {
            ...
            error_log("ADMIN_API: Verificando plano - tipo: $tipo, plano_id: " . var_export($plano_id, true));
            if ($tipo === 'infoprodutor' && $plano_id !== null && $plano_id > 0) {
                error_log("ADMIN_API: Condições atendidas para atribuir plano");
                if (file_exists(__DIR__ . '/../saas/includes/saas_functions.php')) {
                    require_once __DIR__ . '/../saas/includes/saas_functions.php';
                    if (function_exists('saas_enabled') && saas_enabled()) {
                        error_log("ADMIN_API: SaaS está habilitado, processando plano");
                        try {
                            // Verificar se o plano existe e está ativo
                            $stmt_plano = $pdo->prepare("SELECT id, periodo FROM saas_planos WHERE id = ? AND ativo = 1");
                            $stmt_plano->execute([$plano_id]);
                            ...
                            if ($plano) {
                                error_log("ADMIN_API: Plano encontrado: " . var_export($plano, true));
                                // Cancelar assinaturas ativas anteriores
                                $stmt_cancel = $pdo->prepare("UPDATE saas_assinaturas SET status = 'cancelada' WHERE usuario_id = ? AND status = 'ativa'");
                                $stmt_cancel->execute([$user_id]);
                                error_log("ADMIN_API: Assinaturas anteriores canceladas");
                                ...
                                if ($has_data_vencimento) {
                                    ...
                                    $stmt_assinatura = $pdo->prepare("
                                        ...
                                        VALUES (?, ?, 'ativa', ?)
                                    ");
                                    $stmt_assinatura->execute([$user_id, $plano_id, $data_inicio]);
                                }
                                error_log("ADMIN_API: Plano ID $plano_id atribuído ao usuário ID $user_id com sucesso");
                            } else {
                                error_log("ADMIN_API: Plano ID $plano_id não encontrado ou inativo");
                                $warning_msg = "Plano selecionado não foi encontrado ou está inativo.";
                            }
                        } catch (PDOException $e) {
                            error_log("ADMIN_API: Erro ao atribuir plano: " . $e->getMessage());
                            error_log("ADMIN_API: Stack trace: " . $e->getTraceAsString());
                            $warning_msg = "Erro ao atribuir plano: " . $e->getMessage();
                            // Não falha a atualização do usuário se houver erro no plano
                        }
                    } else {
                        error_log("ADMIN_API: SaaS não está habilitado");
                        $warning_msg = "Sistema SaaS não está habilitado.";
                    }
                } else {
                    error_log("ADMIN_API: Arquivo saas_functions.php não encontrado");
                    $warning_msg = "Sistema SaaS não está configurado.";
                }
            } elseif ($tipo === 'infoprodutor' && ($plano_id === null || $plano_id === 0)) {
                // Se plano_id for null/vazio, cancelar assinaturas ativas (remover plano)
                if (file_exists(__DIR__ . '/../saas/includes/saas_functions.php')) {
                    require_once __DIR__ . '/../saas/includes/saas_functions.php';
                    if (function_exists('saas_enabled') && saas_enabled()) {
                        try {
                            // Compatibilidade: algumas instalações antigas usavam status 'ativo' para assinaturas ativas
                            $stmt_cancel = $pdo->prepare("UPDATE saas_assinaturas SET status = 'cancelado' WHERE usuario_id = ? AND status IN ('ativa','ativo')");
                            $stmt_cancel->execute([$user_id]);
                ...
                error_log("ADMIN_API: Condições não atendidas - tipo: $tipo, plano_id: " . var_export($plano_id, true));
            }
    ...
    elseif ($action === 'get_email_settings') {
        ...
        $nome_plataforma_default = getSystemSetting('nome_plataforma', 'CheckoutPRO');
        ...
        $logo_url_raw = getSystemSetting('logo_url', 'https://rafaelsouzatech.com.br/wp-content/uploads/2026/01/flixmembers-31-10-2025.webp');
            ...
            <body>
                <div class="container">
                    ...
                    <div class="footer">
                        ...
                        <p>&copy; ' . date("Y") . ' ' . getSystemSetting('nome_plataforma', 'CheckoutPRO') . '. Todos os direitos reservados.</p>
                    </div>
                </div>
            </body>
            </html>
        ...
        // Substitui "CheckoutPRO" pelo nome dinâmico da plataforma no template HTML (se existir)
        $email_template_html = $configs['email_template_delivery_html'] ?? $default_html_template;
        $nome_plataforma_atual = getSystemSetting('nome_plataforma', 'CheckoutPRO');
        ...
        $logo_url_raw_db = getSystemSetting('logo_url', 'https://rafaelsouzatech.com.br/wp-content/uploads/2026/01/flixmembers-31-10-2025.webp');
        ...
        // Substitui ocorrências de "CheckoutPRO" no copyright do template pelo nome dinâmico
        // Procura por padrões como "© YYYY CheckoutPRO" ou "CheckoutPRO. Todos os direitos"
        // Formato 1: &copy; YYYY CheckoutPRO. Todos os direitos reservados.
        $email_template_html = preg_replace(
            '/(&copy;|©)\s*(\d{4})\s+CheckoutPRO\s*\.\s*Todos os direitos reservados\./i',
            '$1 $2 ' . htmlspecialchars($nome_plataforma_atual) . '. Todos os direitos reservados.',
            $email_template_html
        );
        // Formato 2: &copy; YYYY CheckoutPRO.
        $email_template_html = preg_replace(
            '/(&copy;|©)\s*(\d{4})\s+CheckoutPRO\s*\./i',
            '$1 $2 ' . htmlspecialchars($nome_plataforma_atual) . '.',
            $email_template_html
        );
        // Formato 3: CheckoutPRO © YYYY
        $email_template_html = preg_replace(
            '/CheckoutPRO\s*(&copy;|©)\s*(\d{4})/i',
    ...
    elseif ($action === 'send_test_email' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Body = 'Olá! Este é um e-mail de teste enviado da sua configuração SMTP na plataforma CheckoutPRO. Se você recebeu esta mensagem, suas configurações estão funcionando corretamente.';
            $mail->AltBody = 'Olá! Este é um e-mail de teste enviado da sua configuração SMTP na plataforma CheckoutPRO. Se você recebeu esta mensagem, suas configurações estão funcionando corretamente.';
    ...
    elseif ($action === 'get_system_settings') {
        ...
        if (empty($logo_url_normalized)) {
            $logo_url_normalized = 'https://rafaelsouzatech.com.br/wp-content/uploads/2026/01/flixmembers-31-10-2025.webp';
    ...
    elseif ($action === 'save_system_settings' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($data['login_bg_url'])) {
            $login_bg_val = filter_var($data['login_bg_url'], FILTER_SANITIZE_URL);
            if (setSystemSetting('login_bg_url', $login_bg_val)) {
                $updated[] = 'login_bg_url';
            } else {
                error_log("ADMIN_API: Erro ao salvar login_bg_url");
            }
        }
        if (isset($data['allow_video_upload'])) {
            $val = $data['allow_video_upload'];
    ...
    elseif ($action === 'get_vapid_keys' && $_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            if ($vapidKeys && !empty($vapidKeys['publicKey'])) {
                ...
                // Verifica possíveis causas do erro
                $errorMsg = 'Erro ao obter ou gerar chaves VAPID.';
                $suggestions = [];
                
                // Verifica se OpenSSL tem suporte a curvas elípticas
                $curves = function_exists('openssl_get_curve_names') ? openssl_get_curve_names() : [];
                if (empty($curves) || !in_array('prime256v1', $curves)) {
                    $suggestions[] = 'OpenSSL não tem suporte a curvas elípticas (prime256v1)';
                }
                
                // Verifica se a biblioteca web-push está disponível
                if (!class_exists('Minishlink\WebPush\VAPID')) {
                    $suggestions[] = 'Biblioteca web-push não está instalada';
                }
                ...
                if ($lastError && (strpos($lastError['message'], 'Unable to create the key') !== false || 
                    ...
                    $suggestions[] = 'Erro OpenSSL ao criar chave EC';
                }
                
                if (!empty($suggestions)) {
                    $errorMsg .= ' Possíveis causas: ' . implode('; ', $suggestions) . '.';
                }
                
                $errorMsg .= ' Acesse /pwa/generate_vapid_keys.php para gerar manualmente.';
    ...
    header('Content-Type: application/json');
    echo json_encode(['error' => 'Ocorreu um erro interno no servidor.', 'message' => $e->getMessage(), 'users' => [], 'pagination' => ['currentPage' => 1, 'totalPages' => 1, 'totalRecords' => 0]]);
    exit;
}