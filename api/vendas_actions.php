...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    if (in_array($type, ['link', 'link_externo'], true)) {
        return ['success' => !empty($content), 'product_name' => $product_data['produto_nome'], 'content_type' => 'link', 'content_value' => $content];
    }
    if (in_array($type, ['email_pdf', 'arquivo'], true)) {
        $safe_content = !empty($content) ? basename((string)$content) : '';
        $pdf_path = __DIR__ . '/../uploads/' . $safe_content;
        return [
            'success' => (!empty($safe_content) && file_exists($pdf_path) && is_readable($pdf_path)),
            'product_name' => $product_data['produto_nome'],
            'content_type' => 'pdf',
            'content_value' => $pdf_path
        ];
    }
...
function manual_send_email($to_email, $customer_name, $products, $password, $login_url, $address_data = null, $setup_token = null) {
    ...
    try {
        ...
        $mail->setFrom($fromEmail, $conf['smtp_from_name'] ?? 'CheckoutPRO');
        $mail->CharSet = 'UTF-8';
        $mail->addAddress($to_email, $customer_name); 
        $subject_from_db = $conf['email_template_delivery_subject'] ?? '';
