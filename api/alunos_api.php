...
switch ($action) {
    ...
    case 'create':
        ...
        try {
            ...
            $stmt_user = $pdo->prepare("SELECT id, nome, senha FROM usuarios WHERE usuario = ? AND tipo = 'usuario'");
    ...
    case 'bulk_create':
        ...
        $lista = str_replace(["\r\n", "\r"], "\n", $lista);
        if (trim($lista) === '') {
            sendJsonResponse(false, ['error' => 'Informe pelo menos uma linha no formato email,nome.'], 400);
        }
        ...
        try {
            ...
            $stmt_check_access = $pdo->prepare("SELECT id FROM alunos_acessos WHERE aluno_email = ? AND produto_id = ?");
            $stmt_insert_access = $pdo->prepare("INSERT INTO alunos_acessos (aluno_email, produto_id) VALUES (?, ?)");
            $stmt_user = $pdo->prepare("SELECT id, nome, senha FROM usuarios WHERE usuario = ? AND tipo = 'usuario'");
            $stmt_create_user = $pdo->prepare("INSERT INTO usuarios (usuario, nome, senha, tipo) VALUES (?, ?, ?, 'usuario')");
            $stmt_update_nome = $pdo->prepare("UPDATE usuarios SET nome = ? WHERE id = ?");
            ...
            foreach ($lines as $idx => $rawLine) {
                ...
                // Esperado: email,nome (primeira vírgula separa)
                $parts = explode(',', $line, 2);
                if (count($parts) < 2) {
                    $erros[] = ['linha' => $idx + 1, 'conteudo' => $rawLine, 'erro' => 'Formato inválido. Use: email,nome'];
                    continue;
                }
                $email = trim($parts[0]);
                $nome = trim($parts[1]);
                ...
                if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    $erros[] = ['linha' => $idx + 1, 'conteudo' => $rawLine, 'erro' => 'Email inválido.'];
                    continue;
                }
                if ($nome === '') {
                    $erros[] = ['linha' => $idx + 1, 'conteudo' => $rawLine, 'erro' => 'Nome é obrigatório.'];
                    continue;
                }

                try {
                    // Já tem acesso?
                    $stmt_check_access->execute([$email, $produto_id]);
                    ...
                    // Insere acesso do aluno
                    $stmt_insert_access->execute([$email, $produto_id]);
                    ...
                    if (!$existing_user) {
                        ...
                        // Token de criação de senha (se disponível)
                        if (file_exists(__DIR__ . '/../helpers/password_setup_helper.php')) {
                            require_once __DIR__ . '/../helpers/password_setup_helper.php';
                            if (function_exists('generate_setup_token')) {
                                $setup_token = generate_setup_token($new_user_id);
                    ...
                    // Envia email se solicitado (fora da transação)
                    if ($enviar_email) {
                        if (file_exists(__DIR__ . '/../helpers/alunos_helper.php')) {
                            require_once __DIR__ . '/../helpers/alunos_helper.php';
                            if (function_exists('send_student_access_email')) {
                                $sent = send_student_access_email($email, $nome, $produto_id, $setup_token);
                                if ($sent) $emails_enviados++;
                            }
                        }
                    }
                } catch (PDOException $e) {
                    if ($pdo->inTransaction()) $pdo->rollBack();
                    $erros[] = ['linha' => $idx + 1, 'conteudo' => $rawLine, 'erro' => 'Erro de banco ao inserir este aluno.'];
                    error_log("ALUNOS_API bulk_create: DB error linha " . ($idx + 1) . " - " . $e->getMessage());
                    continue;
                } catch (Throwable $e) {
                    if ($pdo->inTransaction()) $pdo->rollBack();
                    $erros[] = ['linha' => $idx + 1, 'conteudo' => $rawLine, 'erro' => 'Erro interno ao inserir este aluno.'];
                    error_log("ALUNOS_API bulk_create: erro linha " . ($idx + 1) . " - " . $e->getMessage());
                    continue;
                }
            }
            ...
            sendJsonResponse(true, [
                ...
                'resultado' => [
                    ...
                    'criados' => $criados,
                    'ja_tinham_acesso' => $ja_tinham_acesso,
            ...
            error_log("Erro de DB ao bulk_create alunos: " . $e->getMessage());
            sendJsonResponse(false, ['error' => 'Erro interno do servidor ao inserir alunos em massa.'], 500);
        }
        break;
