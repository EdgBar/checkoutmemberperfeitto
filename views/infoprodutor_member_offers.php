...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar_ofertas'])) {
    ...
    try {
        ...
        // 2. Inserir as novas ofertas ativas (apenas aquelas que foram marcadas), com link Saber Mais opcional
        if (isset($_POST['active_offers']) && is_array($_POST['active_offers'])) {
            $saber_mais_post = isset($_POST['saber_mais']) && is_array($_POST['saber_mais']) ? $_POST['saber_mais'] : [];
            $stmt_insert = $pdo->prepare("INSERT INTO product_exclusive_offers (source_product_id, offer_product_id, is_active, saber_mais_url) VALUES (?, ?, 1, ?)");
            foreach ($_POST['active_offers'] as $offer_product_id) {
                ...
                if ($stmt_check_offer_product->rowCount() > 0) {
                    $saber_mais_url = isset($saber_mais_post[$offer_product_id]) ? trim((string) $saber_mais_post[$offer_product_id]) : '';
                    if ($saber_mais_url !== '') {
                        if (strpos($saber_mais_url, 'http') !== 0 && strpos($saber_mais_url, 'wa.me') !== 0 && strpos($saber_mais_url, 'api.whatsapp') !== 0) {
                            if (preg_match('/^[\d\s\-\(\)]+$/', $saber_mais_url)) {
                                $saber_mais_url = 'https://wa.me/55' . preg_replace('/\D/', '', $saber_mais_url);
                            } else {
                                $saber_mais_url = 'https://' . $saber_mais_url;
                            }
                        }
                    } else {
                        $saber_mais_url = null;
                    }
                    $stmt_insert->execute([$source_product_id, $offer_product_id, $saber_mais_url === null ? null : $saber_mais_url]);
                }
            }
        }
...
// 4. Buscar as ofertas ativas existentes para este source_product_id (inclui link Saber Mais)
$stmt_current_offers = $pdo->prepare("
    SELECT offer_product_id, COALESCE(saber_mais_url, '') AS saber_mais_url
    FROM product_exclusive_offers
    WHERE source_product_id = ? AND is_active = 1
");
$stmt_current_offers->execute([$source_product_id]);
$current_offers_rows = $stmt_current_offers->fetchAll(PDO::FETCH_ASSOC);
$current_offer_product_ids = array_column($current_offers_rows, 'offer_product_id');
$current_saber_mais = [];
foreach ($current_offers_rows as $row) {
    $current_saber_mais[$row['offer_product_id']] = $row['saber_mais_url'] ?? '';
}

// 5. Combinar os dados: Adicionar flag 'is_offer' e 'saber_mais_url' a cada produto potencial
foreach ($potential_offer_products as &$product) {
    $product['is_offer'] = in_array($product['id'], $current_offer_product_ids);
    $product['saber_mais_url'] = $current_saber_mais[$product['id']] ?? '';
}
                        ...
                        <div class="group bg-dark-elevated rounded-lg overflow-hidden border border-dark-border hover:shadow-lg transition-shadow duration-300 flex flex-col">
                            ...
                            <div class="p-4 flex-grow flex flex-col justify-between">
                                ...
                                <div class="mt-3 space-y-3">
                                    ...
                                    <div>
                                        <label for="saber-mais-<?php echo $product['id']; ?>" class="block text-xs font-medium text-gray-400 mb-1">Link Saber Mais (opcional)</label>
                                        <input type="text" id="saber-mais-<?php echo $product['id']; ?>" name="saber_mais[<?php echo $product['id']; ?>]" value="<?php echo htmlspecialchars($product['saber_mais_url'] ?? ''); ?>" placeholder="URL ou WhatsApp" class="w-full px-3 py-2 bg-dark-card border border-dark-border rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-[#32e768]">
                                        <p class="text-xs text-gray-500 mt-1">Página de vendas ou link do WhatsApp. Se preenchido, o aluno verá o botão "Saber Mais" além do "Comprar".</p>
                                    </div>
                                </div>
                            </div>
                        </div>
