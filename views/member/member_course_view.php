...
<head>
    ...
    <style>
        ...
        .lesson-item.active { background-color: color-mix(in srgb, var(--accent-primary) 35%, transparent); color: #fff; font-weight: 600; border-left-color: var(--accent-primary); }
        .lesson-item.active .lucide-play-circle { color: var(--accent-primary); }
    ...
    <header class="text-white p-6 shadow-md sticky top-0 z-50" style="background-color: #07090d;">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <h1 class="text-2xl font-bold"><?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['titulo'] ?? 'Detalhes do Curso'); ?></h1>
            </div>
            <div class="flex items-center space-x-4">
    ...
    <div id="course-container" class="min-h-screen">
        ...
        <header class="relative h-64 md:h-80 bg-gray-800 bg-cover bg-center" style="background-image: url('<?php echo htmlspecialchars($curso['banner_url'] ?? ($upload_dir . $curso['produto_foto'] ?? '')); ?>')">
            ...
            <div class="relative h-full flex flex-col justify-end p-6 md:p-10 max-w-7xl mx-auto">
                <h1 class="text-3xl md:text-5xl font-extrabold text-white drop-shadow-lg"><?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['titulo'] ?? ''); ?></h1>
                <p class="mt-2 text-lg text-gray-300 max-w-2xl drop-shadow-md"><?php echo htmlspecialchars($curso['produto_descricao'] ?? $curso['descricao'] ?? ''); ?></p>
            </div>
        </header>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function loadLesson(lesson) {
                ...
                // 3. Lógica de exibição: YouTube ou vídeo MP4 (upload na plataforma)
                let videoId = null;
                let isShort = false;
                const isMp4Upload = lesson.url_video && (lesson.url_video.indexOf('uploads/') === 0 || lesson.url_video.toLowerCase().endsWith('.mp4'));
                if ((lesson.tipo_conteudo === 'video' || lesson.tipo_conteudo === 'mixed') && lesson.url_video && !isMp4Upload) {
                ...
                // 4. Carrega o Player: YMin (YouTube), HTML5 (MP4) ou placeholder
                if (videoId) {
                    ...
                    const videoUrl = (lesson.url_video.indexOf('/') === 0) ? lesson.url_video : '/' + lesson.url_video;
                    playerHost.innerHTML = `<div class="w-full aspect-video bg-black rounded-xl overflow-hidden">
                        <video class="w-full h-full" controls playsinline preload="metadata" src="${videoUrl.replace(/"/g, '&quot;')}">
