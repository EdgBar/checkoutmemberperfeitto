...
<head>
    ...
    <title>Meus Cursos - Área de Membros CheckoutPRO</title>
    <?php
            ...
            <div class="flex justify-between items-center h-20">
                <div class="flex items-center space-x-4">
                    <a href="/member_area_dashboard">
                        ...
                        ?>
                        <img src="<?php echo htmlspecialchars($logo_url); ?>" alt="<?php echo htmlspecialchars(getSystemSetting('nome_plataforma', 'CheckoutPRO')); ?> Logo" class="h-10">
                    </a>
                </div>
                    ...
                    <div class="group bg-gray-800 rounded-2xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:scale-[1.02] border border-gray-700/50 flex flex-col relative">
                        ...
                        <div class="relative aspect-video overflow-hidden">
                            ...
                            <img src="<?php echo htmlspecialchars($image_path); ?>" 
                                 alt="<?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['curso_titulo'] ?? ''); ?>"
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            <h3 class="text-2xl font-bold text-white mb-3 line-clamp-2">
                                <?php echo htmlspecialchars($curso['produto_nome'] ?? $curso['curso_titulo'] ?? 'Curso'); ?>
                            </h3>
                            <p class="text-gray-400 text-sm mb-4 line-clamp-3 flex-grow">
                                <?php echo htmlspecialchars($curso['produto_descricao'] ?? $curso['curso_descricao'] ?? 'Acesse para ver mais detalhes.'); ?>
                            </p>
                ...
                try {
                    ...
                    if (data.offers && data.offers.length > 0) {
                        data.offers.forEach(offer => {
                            ...
                            const saberMaisUrl = (offer.saber_mais_url || '').trim();

                            const saberMaisEscaped = saberMaisUrl ? saberMaisUrl.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&#39;') : '';
                            const botoesHtml = saberMaisUrl
                                ? `
                                <div class="mt-4 flex flex-col gap-2">
                                    <a href="${saberMaisEscaped}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center bg-gray-600 hover:bg-gray-500 text-white font-bold py-2.5 px-4 rounded-lg transition duration-300 text-sm" onclick="event.stopPropagation();">
                                        <i data-lucide="info" class="w-4 h-4 mr-2"></i>
                                        Saber Mais
                                    </a>
                                    <a href="${checkoutLink}" class="inline-flex items-center justify-center bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-5 rounded-lg transition duration-300 text-base">
                                        Comprar por ${productPrice}
                                    </a>
                                </div>
                                `
                                : `
                                <a href="${checkoutLink}" class="mt-4 inline-flex items-center justify-center bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-5 rounded-lg transition duration-300 text-base block">
                                    Comprar por ${productPrice}
                                </a>
                                `;

                            slide.innerHTML = `
                                <div class="offer-card-style offer-card relative block h-full flex flex-col">
                                    <div class="lock-icon">
