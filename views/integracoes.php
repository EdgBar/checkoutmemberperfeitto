...
try {
    $stmt_user_data = $pdo->prepare("SELECT mp_public_key, mp_access_token, pushinpay_token, efi_client_id, efi_client_secret, efi_certificate_path, efi_pix_key, efi_payee_code, hypercash_secret_key, hypercash_public_key, stripe_secret_key, stripe_publishable_key FROM usuarios WHERE id = ?");
    ...
    if (strpos($e->getMessage(), 'efi_client_id') !== false || strpos($e->getMessage(), 'hypercash_secret_key') !== false || strpos($e->getMessage(), 'Column not found') !== false) {
        ...
        try {
            ...
            $user_data_fetched['stripe_publishable_key'] = null;
        }
    } else {
...
$stripe_configured = !empty($stripe_secret_key) && !empty($stripe_publishable_key);
...
if (isset($_POST['salvar_gateways'])) {
    ...
    if (isset($_FILES['efi_certificate']) && $_FILES['efi_certificate']['error'] === UPLOAD_ERR_OK) {
        ...
        try {
            try {
                $stmt = $pdo->prepare("UPDATE usuarios SET mp_public_key = ?, mp_access_token = ?, pushinpay_token = ?, efi_client_id = ?, efi_client_secret = ?, efi_certificate_path = ?, efi_pix_key = ?, efi_payee_code = ?, hypercash_secret_key = ?, hypercash_public_key = ?, stripe_secret_key = ?, stripe_publishable_key = ? WHERE id = ?");
                $stmt->execute([$public_key, $access_token, $pp_token, $efi_client_id, $efi_client_secret, $certificate_path, $efi_pix_key, $efi_payee_code, $hypercash_secret_key, $hypercash_public_key, $stripe_secret_key, $stripe_publishable_key, $usuario_id_logado]);
            } catch (PDOException $e_col) {
                if (strpos($e_col->getMessage(), 'stripe_secret_key') !== false || strpos($e_col->getMessage(), 'Column not found') !== false) {
...
<body class="min-h-screen text-white pb-20">
        ...
        <form action="/index?pagina=integracoes" method="post" enctype="multipart/form-data" class="space-y-8 mt-8">
            ...
            <div id="gateway-selection-cards" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 hidden">
                ...
                <div id="card-stripe" onclick="showGateway('stripe')"
                    ...
                    <div class="flex items-center gap-5 mb-4">
                        <div class="h-16 w-16 rounded-2xl bg-indigo-500/20 flex items-center justify-center border border-indigo-400/30 shadow-sm group-hover:bg-indigo-500/30 transition-colors">
                            <svg class="h-8 w-8 text-indigo-400" viewBox="0 0 24 24" fill="currentColor"><path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.549-2.354 1.549-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.571-7.307z"/></svg>
                        </div>
                        <div>
                            <h3 class="text-xl font-bold text-white group-hover:text-indigo-300 transition-colors">Stripe</h3>
                            <p class="text-gray-400 text-sm">Cartão de Crédito (internacional)</p>
                        </div>
                    </div>
                ...
                <div class="bg-dark-card rounded-2xl shadow-xl border border-dark-border overflow-hidden">
                    ...
                    <div class="p-8 md:p-10">
                        ...
                        <div id="fields-stripe" class="hidden gateway-section">
                            <div class="flex items-center gap-4 mb-8 border-b border-dark-border pb-6">
                                <div class="h-12 w-12 rounded-xl bg-indigo-500/20 flex items-center justify-center text-indigo-300 border border-indigo-400/30">
                                    <i data-lucide="key" class="w-6 h-6"></i>
                                </div>
                                <div>
                                    <h3 class="text-xl font-bold text-white">Credenciais Stripe</h3>
                                    <p class="text-gray-400">Insira suas chaves de produção (Live Keys) para cartão de crédito.</p>
                                </div>
                            </div>

                            <div class="grid gap-6">
                                <div class="group">
                                    <label class="block text-sm font-semibold text-gray-300 mb-2">Publishable Key</label>
                                    <div class="custom-input flex items-center border border-dark-border rounded-lg px-4 py-3 bg-dark-elevated transition-all">
                                        <i data-lucide="unlock" class="text-gray-400 w-5 h-5 mr-3"></i>
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">Chave pública do Stripe usada no frontend (começa com <code>pk_</code>).</p>
                                </div>
                                ...
                                <div class="group">
                                    ...
                                    <p class="text-xs text-gray-400 mt-1">Chave secreta do Stripe usada no backend (começa com <code>sk_</code>).</p>
                                </div>
                            </div>
                            ...
                            <div class="rounded-xl bg-indigo-900/20 border border-indigo-500/30 p-4 flex gap-4 items-start mt-6">
                                <i data-lucide="info" class="text-indigo-400 w-5 h-5 flex-shrink-0 mt-0.5"></i>
                                <div>
                                    <h4 class="text-sm font-bold text-indigo-300">Importante</h4>
                                    <p class="text-sm text-indigo-200 mt-1">
                                        Use chaves de <strong>produção</strong> (Live). Em ambiente de testes, você pode usar chaves <code>pk_test_</code>/<code>sk_test_</code>.
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function showGateway(gateway) {
            // Visual reset
            const allCards = document.querySelectorAll('#card-mp, #card-pp, #card-efi, #card-hypercash, #card-stripe');
