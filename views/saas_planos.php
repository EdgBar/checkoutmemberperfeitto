...
// Separar planos por período (incluindo planos gratuitos)
$planos_mensais = array_filter($planos, function($p) { 
    return $p['periodo'] === 'mensal'; 
});
$planos_anuais = array_filter($planos, function($p) { 
    return $p['periodo'] === 'anual'; 
});

// Buscar plano atual do usuário (recarregar se success=1 para mostrar atualizado)
$plano_atual = null;
...
<div class="container mx-auto">
    ...
    <div id="success-message" class="bg-green-900/20 border border-green-500 text-green-300 px-4 py-3 rounded relative mb-6" role="alert">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <i data-lucide="check-circle" class="w-6 h-6 text-green-400"></i>
                <div>
                    <strong class="font-bold">Plano Ativado com Sucesso!</strong>
                    <p class="text-sm mt-1">Seu plano foi ativado e você já pode começar a usar todas as funcionalidades.</p>
                </div>
            </div>
            <button onclick="document.getElementById('success-message').remove()" class="text-green-400 hover:text-green-300">
                ...
                <?php foreach ($planos_anuais as $plano): ?>
                    <div class="bg-dark-card rounded-xl border border-dark-border p-6 hover:border-primary/50 transition-all <?php echo $plano_atual == $plano['id'] ? 'ring-2 ring-primary' : ''; ?>">
                        ...
                        <?php if ($plano_atual != $plano['id']): ?>
                            <?php 
                            $checkout_url = '/saas/checkout/checkout_plano.php?plano_id=' . intval($plano['id']);
                            $is_free_plan = ($plano['is_free'] == 1 || floatval($plano['preco']) == 0);
                            ?>
                            <a href="<?php echo htmlspecialchars($checkout_url); ?>" 
                               class="block w-full text-center px-6 py-3 bg-primary hover:bg-primary/80 text-white rounded-lg font-semibold transition-colors">
                                <?php echo $is_free_plan ? 'Ativar Plano Grátis' : 'Assinar Agora'; ?>
                            </a>
                        <?php else: ?>
                            <button disabled class="block w-full text-center px-6 py-3 bg-gray-600 text-gray-400 rounded-lg font-semibold cursor-not-allowed">
                                Plano Atual
...
// Se houver mensagem de sucesso, garantir que os ícones sejam criados
