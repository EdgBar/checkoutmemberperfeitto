    ...
    if ($plano_info) {
        try {
            $stmt = $pdo->prepare("
                ...
                WHERE sa.usuario_id = ? AND sa.status = 'ativa' AND sa.data_vencimento > NOW()
            ...
            <?php else: ?>
                <?php foreach ($planos_mensais as $plano): ?>
                    <div class="bg-dark-card rounded-xl border border-dark-border p-6 hover:border-primary/50 transition-all <?php echo $plano_atual == $plano['id'] ? 'ring-2 ring-primary' : ''; ?>">
                        ...
                        <?php elseif ($plano['preco'] == 0 || $plano['is_free'] == 1): ?>
                            <a href="/saas/checkout/checkout_plano.php?plano_id=<?php echo $plano['id']; ?>" class="block w-full bg-primary hover:bg-primary-hover text-white font-semibold py-3 px-6 rounded-lg transition-colors text-center">
                                Ativar Plano Grátis
                            </a>
                        <?php else: ?>
                            <a href="/saas/checkout/checkout_plano.php?plano_id=<?php echo $plano['id']; ?>" class="block w-full bg-primary hover:bg-primary-hover text-white font-semibold py-3 px-6 rounded-lg transition-colors text-center">
