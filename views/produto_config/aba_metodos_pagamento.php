...
try {
    $stmt_credenciais = $pdo->prepare("SELECT mp_access_token, pushinpay_token, efi_client_id, efi_client_secret, efi_certificate_path, efi_pix_key, efi_payee_code, hypercash_secret_key, hypercash_public_key, stripe_secret_key, stripe_publishable_key FROM usuarios WHERE id = ?");
    ...
    try {
        ...
        $credenciais['stripe_publishable_key'] = null;
...
// Para Stripe, verificar se ambas as chaves estão preenchidas
$stripe_configured = !empty($credenciais['stripe_secret_key'] ?? '') && !empty($credenciais['stripe_publishable_key'] ?? '');
    ...
    <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-dark-elevated p-6 rounded-lg border border-dark-border <?php echo !$stripe_configured ? 'opacity-50 pointer-events-none' : ''; ?>">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-white flex items-center gap-2">
                        <svg class="w-5 h-5 text-indigo-400" viewBox="0 0 24 24" fill="currentColor"><path d="M13.976 9.15c-2.172-.806-3.356-1.426-3.356-2.409 0-.831.683-1.305 1.901-1.305 2.227 0 4.515.858 6.09 1.631l.89-5.494C18.252.975 15.697 0 12.165 0 9.667 0 7.589.654 6.104 1.872 4.56 3.147 3.757 4.992 3.757 7.218c0 4.039 2.467 5.76 6.476 7.219 2.585.92 3.445 1.574 3.445 2.583 0 .98-.84 1.549-2.354 1.549-1.875 0-4.965-.921-6.99-2.109l-.9 5.555C5.175 22.99 8.385 24 11.714 24c2.641 0 4.843-.624 6.328-1.813 1.664-1.305 2.525-3.236 2.525-5.732 0-4.128-2.524-5.851-6.571-7.307z"/></svg>
                        Stripe
                    </h3>
                ...
                <div class="space-y-3">
                    <label class="flex items-start p-3 bg-dark-card border border-dark-border rounded-lg cursor-pointer transition-all hover:border-indigo-500" <?php echo !$stripe_configured ? 'style="pointer-events: none;"' : ''; ?>>
                        ...
                        <div class="ml-3 flex-1">
                            <div class="flex items-center gap-2">
                                <span class="text-sm font-medium text-white">Cartão de Crédito</span>
                                <span class="bg-indigo-900/30 text-indigo-400 text-xs font-bold px-2 py-0.5 rounded">Internacional</span>
                            </div>
                            <p class="text-xs text-gray-400 mt-1">Visa, Mastercard, etc. (Stripe)</p>
                        </div>
                    </label>
                </div>
            </div>
...
function enforceSingleCreditCard() {
    ...
    const uncheck = (el, name) => {
        if (!el) return;
        el.checked = false;
        const h = document.querySelector('input[name="' + name + '"][type="hidden"]');
        if (h) h.value = '0';
    };

    // Prioridade: Stripe > Hypercash > Efí > Mercado Pago
    if (creditCardStripe && creditCardStripe.checked && !creditCardStripe.disabled) {
        uncheck(creditCardMP, 'payment_credit_card_mercadopago');
        uncheck(creditCardHypercash, 'payment_credit_card_hypercash');
        uncheck(creditCardEfi, 'payment_credit_card_efi');
    } else if (creditCardHypercash && creditCardHypercash.checked && !creditCardHypercash.disabled) {
        uncheck(creditCardMP, 'payment_credit_card_mercadopago');
        uncheck(creditCardEfi, 'payment_credit_card_efi');
        uncheck(creditCardStripe, 'payment_credit_card_stripe');
