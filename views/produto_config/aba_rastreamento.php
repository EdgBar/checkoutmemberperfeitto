...
// Sanitizar valores para evitar bloqueio do WAF
$safe_custom_script = base64_encode($tracking_config['customScript'] ?? '');
?>
    ...
    <div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-dark-elevated p-6 rounded-lg border border-dark-border">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="customScriptVisible" class="block text-gray-300 text-sm font-semibold mb-2">Script de Rastreamento Personalizado</label>
                        <!-- Campo visível para o usuário editar -->
                        <textarea id="customScriptVisible" rows="8" class="form-input font-mono text-sm" placeholder="Cole aqui o código de rastreamento"></textarea>
                        <!-- Campo hidden que será enviado com o valor em base64 -->
                        <input type="hidden" id="customScriptB64" name="customScriptB64" value="<?php echo $safe_custom_script; ?>">
                        <p class="text-xs text-gray-400 mt-2">
                            Este código será inserido automaticamente no HEAD da página de checkout e da página de obrigado.
                        </p>
                        <script>
                        (function(){
                            // Decodifica o valor base64 para exibir no textarea
                            var b64Input = document.getElementById('customScriptB64');
                            var visibleTextarea = document.getElementById('customScriptVisible');
                            
                            if(b64Input.value) {
                                try {
                                    visibleTextarea.value = decodeURIComponent(escape(atob(b64Input.value)));
                                } catch(e) {
                                    console.error('Erro ao decodificar script:', e);
                                }
                            }
                            
                            // Atualiza o campo hidden em base64 quando o usuário digita
                            visibleTextarea.addEventListener('input', function() {
                                try {
                                    b64Input.value = btoa(unescape(encodeURIComponent(this.value)));
                                } catch(e) {
                                    console.error('Erro ao codificar script:', e);
                                }
                            });
                            
                            // Também atualiza antes do submit do formulário
                            var form = visibleTextarea.closest('form');
                            if(form) {
                                form.addEventListener('submit', function() {
                                    try {
                                        b64Input.value = btoa(unescape(encodeURIComponent(visibleTextarea.value)));
                                    } catch(e) {
                                        console.error('Erro ao codificar script no submit:', e);
                                    }
                                });
                            }
                        })();
