...
try {
    ...
    if ($filtro_status !== 'all') {
        // Mapear status do filtro para valores do banco
        $status_map = [
            'ativo' => 'ativa',
            'pending' => 'trial',
            'pendente' => 'trial',
        ...
        $status_db = $status_map[$filtro_status] ?? $filtro_status;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancelar_assinatura'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE saas_assinaturas SET status = 'cancelada' WHERE id = ?");
        $stmt->execute([$id]);
    ...
    if ($assinatura_id && $novo_plano_id) {
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, 'ativa', CURDATE(), ?, 'Alterado pelo Admin')
            ");
            $stmt->execute([
...
<div class="container mx-auto">
    ...
    <div class="bg-dark-card p-4 rounded-lg shadow-md border mb-6" style="border-color: var(--accent-primary);">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="/admin?pagina=saas_assinaturas&status=ativa<?php echo !empty($pesquisa) ? '&pesquisa=' . urlencode($pesquisa) : ''; ?>" 
               class="px-4 py-2 rounded-lg font-semibold <?php echo $filtro_status === 'ativa' ? 'bg-primary text-white' : 'bg-dark-elevated text-gray-400 hover:text-white'; ?>">
                Ativas
            </a>
            <a href="/admin?pagina=saas_assinaturas&status=trial<?php echo !empty($pesquisa) ? '&pesquisa=' . urlencode($pesquisa) : ''; ?>" 
               class="px-4 py-2 rounded-lg font-semibold <?php echo $filtro_status === 'trial' ? 'bg-primary text-white' : 'bg-dark-elevated text-gray-400 hover:text-white'; ?>">
                Pendentes
            </a>
            <a href="/admin?pagina=saas_assinaturas&status=cancelada<?php echo !empty($pesquisa) ? '&pesquisa=' . urlencode($pesquisa) : ''; ?>" 
               class="px-4 py-2 rounded-lg font-semibold <?php echo $filtro_status === 'cancelada' ? 'bg-primary text-white' : 'bg-dark-elevated text-gray-400 hover:text-white'; ?>">
                Canceladas
            </a>
                        ...
                        <?php foreach ($assinaturas as $assinatura): ?>
                            <tr class="border-b border-dark-border hover:bg-dark-elevated">
                                ...
                                <td class="py-3 px-4">
                                    ...
                                    $status_display = $status_display_map[$assinatura['status']] ?? ucfirst($assinatura['status']);
