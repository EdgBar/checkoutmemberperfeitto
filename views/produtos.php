...
// Garante que o diretório REAL de uploads exista (evita problemas com caminho relativo no Windows/Laragon)
$upload_dir_fs = realpath(__DIR__ . '/..') . DIRECTORY_SEPARATOR . 'uploads';
if ($upload_dir_fs && !is_dir($upload_dir_fs)) {
...
// Duplicar produto
if (isset($_POST['duplicar_produto'])) {
    if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
        ...
        $id_origem = (int)($_POST['id_produto'] ?? 0);
        if ($id_origem <= 0) {
            $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>ID de produto inválido.</div>";
        } else {
            try {
                $stmt_origem = $pdo->prepare("SELECT * FROM produtos WHERE id = ? AND usuario_id = ?");
                $stmt_origem->execute([$id_origem, $usuario_id]);
                $origem = $stmt_origem->fetch(PDO::FETCH_ASSOC);
                if (!$origem) {
                    $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Produto não encontrado ou permissão negada.</div>";
                } else {
                    // Verificação de limite (SaaS) se existir
                    if (function_exists('saas_enabled') && saas_enabled() && file_exists(__DIR__ . '/../saas/saas.php')) {
                        require_once __DIR__ . '/../saas/saas.php';
                    }
                    $limit_check = function_exists('do_action') ? do_action('before_create_product', $usuario_id) : null;
                    ...
                    if ($limit_check && isset($limit_check['allowed']) && $limit_check['allowed'] === false) {
                        ...
                        $stmt_ins->execute([
                            ...
                            $checkout_hash,
                            $origem['tipo_entrega'],
                            $origem['conteudo_entrega'],
                            $usuario_id,
                            $origem['gateway'] ?? 'mercadopago'
                        ]);
                        $novo_id = (int) $pdo->lastInsertId();
                        if ($novo_id > 0) {
                            if (!empty($origem['checkout_config'])) {
                                $stmt_cfg = $pdo->prepare("UPDATE produtos SET checkout_config = ? WHERE id = ? AND usuario_id = ?");
                                $stmt_cfg->execute([$origem['checkout_config'], $novo_id, $usuario_id]);
                            }
                            if (isset($origem['preco_anterior']) && $origem['preco_anterior'] !== null) {
                                $stmt_preco = $pdo->prepare("UPDATE produtos SET preco_anterior = ? WHERE id = ? AND usuario_id = ?");
                                $stmt_preco->execute([$origem['preco_anterior'], $novo_id, $usuario_id]);
                            }
                            if ($origem['tipo_entrega'] === 'area_membros') {
                                $stmt_curso = $pdo->prepare("SELECT * FROM cursos WHERE produto_id = ?");
                                $stmt_curso->execute([$id_origem]);
                                $curso_origem = $stmt_curso->fetch(PDO::FETCH_ASSOC);
                                if ($curso_origem) {
                                    $stmt_curso_ins = $pdo->prepare("INSERT INTO cursos (produto_id, titulo, descricao, imagem_url) VALUES (?, ?, ?, ?)");
                                    $stmt_curso_ins->execute([$novo_id, $curso_origem['titulo'], $curso_origem['descricao'], $curso_origem['imagem_url'] ?? null]);
                                    $novo_curso_id = (int) $pdo->lastInsertId();
                                    if (!empty($curso_origem['banner_url'])) {
                                        @$pdo->prepare("UPDATE cursos SET banner_url = ? WHERE id = ?")->execute([$curso_origem['banner_url'], $novo_curso_id]);
                                    }
                                    $stmt_mod = $pdo->prepare("SELECT * FROM modulos WHERE curso_id = ? ORDER BY id ASC");
                                    $stmt_mod->execute([$curso_origem['id']]);
                                    $modulos_origem = $stmt_mod->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($modulos_origem as $mod) {
                                        $stmt_mod_ins = $pdo->prepare("INSERT INTO modulos (curso_id, titulo, release_days, is_paid_module, linked_product_id) VALUES (?, ?, ?, ?, ?)");
                                        $stmt_mod_ins->execute([
                                            $novo_curso_id,
                                            $mod['titulo'],
                                            $mod['release_days'] ?? 0,
                                            $mod['is_paid_module'] ?? 0,
                                            $mod['linked_product_id'] ?? null
                                        ]);
                                        $novo_modulo_id = (int) $pdo->lastInsertId();
                                        if (!empty($mod['imagem_capa_url'])) {
                                            @$pdo->prepare("UPDATE modulos SET imagem_capa_url = ? WHERE id = ?")->execute([$mod['imagem_capa_url'], $novo_modulo_id]);
                                        }
                                        $stmt_aula = $pdo->prepare("SELECT * FROM aulas WHERE modulo_id = ? ORDER BY ordem ASC, id ASC");
                                        $stmt_aula->execute([$mod['id']]);
                                        $aulas_origem = $stmt_aula->fetchAll(PDO::FETCH_ASSOC);
                                        foreach ($aulas_origem as $aula) {
                                            $stmt_aula_ins = $pdo->prepare("INSERT INTO aulas (modulo_id, titulo, url_video, descricao, release_days, tipo_conteudo) VALUES (?, ?, ?, ?, ?, ?)");
                                            $stmt_aula_ins->execute([
                                                $novo_modulo_id,
                                                $aula['titulo'],
                                                $aula['url_video'] ?? '',
                                                $aula['descricao'] ?? '',
                                            ...
                                            $nova_aula_id = (int) $pdo->lastInsertId();
                                            $stmt_arq = $pdo->prepare("SELECT nome_original, nome_salvo, caminho_arquivo, tipo_mime, tamanho_bytes FROM aula_arquivos WHERE aula_id = ? ORDER BY id ASC");
                                            $stmt_arq->execute([$aula['id']]);
                                            $arquivos = $stmt_arq->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($arquivos as $arq) {
                                                $stmt_arq_ins = $pdo->prepare("INSERT INTO aula_arquivos (aula_id, nome_original, nome_salvo, caminho_arquivo, tipo_mime, tamanho_bytes) VALUES (?, ?, ?, ?, ?, ?)");
                                                $stmt_arq_ins->execute([$nova_aula_id, $arq['nome_original'], $arq['nome_salvo'], $arq['caminho_arquivo'], $arq['tipo_mime'], $arq['tamanho_bytes']]);
                                            }
                                        }
                                    }
                                }
                        ...
                        if (function_exists('do_action')) {
                            do_action('after_create_product', $novo_id, $usuario_id);
                        }
                        $_SESSION['flash_message'] = "<div class='animate-fade-in-down bg-green-900/20 border-l-4 border-green-500 text-green-300 p-4 rounded-md shadow-sm mb-6' role='alert'><div class='flex'><div class='py-1'><i data-lucide='check-circle' class='w-6 h-6 mr-3 text-green-400'></i></div><div><p class='font-bold text-white'>Produto duplicado</p><p class='text-sm text-green-200'>O produto foi duplicado com sucesso. Novo link de checkout gerado.</p></div></div></div>";
                        header("Location: /index?pagina=produtos");
                        exit;
                    }
                }
            } catch (PDOException $e) {
                $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Erro ao duplicar: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
        }
    }
}
...
if (isset($_POST['deletar_produto'])) {
    ...
    if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
        ...
        if ($produto_files) {
            ...
            if (in_array($produto_files['tipo_entrega'], ['email_pdf', 'arquivo'], true) && $produto_files['conteudo_entrega'] && file_exists($upload_dir . $produto_files['conteudo_entrega'])) {
...
if (isset($_POST['salvar_produto'])) {
    ...
    // Robustez: se o arquivo PDF foi enviado, força tipo_entrega = email_pdf
    // (evita casos onde o JS não atualiza o hidden input, mas o usuário fez upload do PDF)
    if (isset($_FILES['conteudo_entrega_pdf']) && is_array($_FILES['conteudo_entrega_pdf'])) {
        if (($_FILES['conteudo_entrega_pdf']['error'] ?? UPLOAD_ERR_NO_FILE) === UPLOAD_ERR_OK) {
            $tipo_entrega = 'arquivo';
        }
    }

    // Compatibilidade com bancos que usam: area_membros | link_externo | arquivo | manual
    // (o sistema antigo usava: link | email_pdf | produto_fisico)
    $tipo_entrega = strtolower(trim((string)$tipo_entrega));
    $map_tipo_entrega = [
        'link' => 'link_externo',
        'link_externo' => 'link_externo',
        'email_pdf' => 'arquivo',
        'arquivo' => 'arquivo',
        'area_membros' => 'area_membros',
        'produto_fisico' => 'manual',
        'manual' => 'manual',
    ];
    $tipo_entrega = $map_tipo_entrega[$tipo_entrega] ?? $tipo_entrega;
    $conteudo_entrega_atual = $_POST['conteudo_entrega_atual'] ?? null;
    $conteudo_entrega = $conteudo_entrega_atual;
    $entrega_valid = true;

    if ($tipo_entrega === 'link_externo') {
        $conteudo_entrega = $_POST['conteudo_entrega_link'] ?? null;
    } elseif ($tipo_entrega === 'area_membros') {
        $conteudo_entrega = null; 
    } elseif ($tipo_entrega === 'manual') {
        $conteudo_entrega = null;
    } elseif ($tipo_entrega === 'arquivo') {
        if (isset($_FILES['conteudo_entrega_pdf']) && is_array($_FILES['conteudo_entrega_pdf'])) {
            $pdf_error = $_FILES['conteudo_entrega_pdf']['error'] ?? UPLOAD_ERR_NO_FILE;
            ...
            if ($pdf_error === UPLOAD_ERR_OK) {
            ...
            } elseif ($pdf_error !== UPLOAD_ERR_NO_FILE) {
                $error_messages = [
                    UPLOAD_ERR_INI_SIZE => 'Arquivo muito grande (excede upload_max_filesize do servidor).',
                    UPLOAD_ERR_FORM_SIZE => 'Arquivo muito grande (excede o limite do formulário).',
                ...
                $mensagem .= "<div class='bg-red-900/20 text-red-300 p-3 rounded mb-4'>Falha ao enviar o PDF: " . htmlspecialchars($error_messages[$pdf_error] ?? ('Erro no upload (código ' . (int)$pdf_error . ')')) . "</div>";
            }
        }

        // Se o usuário selecionou PDF, é obrigatório ter um arquivo salvo (novo ou atual)
        if (empty($conteudo_entrega)) {
            $entrega_valid = false;
            $mensagem .= "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Você selecionou <strong>Arquivo PDF</strong> como entrega, mas nenhum PDF foi enviado/salvo. Envie um arquivo PDF (até 10MB) e tente novamente.</div>";
        }
    }

    try {
        if (!$entrega_valid) {
            // Não salva produto se configuração de entrega estiver inválida
            throw new Exception('Configuração de entrega inválida');
        }
        if (empty($id_produto)) {
            ...
            // Já adicionamos mensagem amigável acima quando aplicável
            if (empty($mensagem)) {
                $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Erro ao salvar o produto.</div>";
            }
        }
    }
}
                ...
                <div class="md:col-span-8 space-y-6">
                    ...
                    <div class="bg-dark-elevated p-6 rounded-xl border border-dark-border">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="hidden" id="tipo_entrega" name="tipo_entrega" value="<?php echo htmlspecialchars($produto_edit['tipo_entrega'] ?? 'link_externo'); ?>">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" style="position: relative; z-index: 1;">
                                <div class="entrega-option-card cursor-pointer p-4 rounded-lg border-2 transition-all duration-200 bg-dark-card hover:bg-dark-elevated <?php echo (($produto_edit['tipo_entrega'] ?? 'link_externo') == 'link_externo') ? 'border-[var(--accent-primary)] bg-dark-elevated' : 'border-dark-border'; ?>" data-value="link_externo" onclick="if(typeof window.selectEntregaOption === 'function') { window.selectEntregaOption('link_externo'); } else { console.error('selectEntregaOption não está definida'); }" style="position: relative; z-index: 2; pointer-events: auto;">
                                ...
                                <div class="entrega-option-card cursor-pointer p-4 rounded-lg border-2 transition-all duration-200 bg-dark-card hover:bg-dark-elevated <?php echo (($produto_edit['tipo_entrega'] ?? '') == 'arquivo') ? 'border-[var(--accent-primary)] bg-dark-elevated' : 'border-dark-border'; ?>" data-value="arquivo" onclick="if(typeof window.selectEntregaOption === 'function') { window.selectEntregaOption('arquivo'); } else { console.error('selectEntregaOption não está definida'); }" style="position: relative; z-index: 2; pointer-events: auto;">
                                    <div class="flex flex-col items-center text-center space-y-2">
                                        <div class="w-12 h-12 rounded-lg flex items-center justify-center mb-2" style="background-color: color-mix(in srgb, var(--accent-primary) 20%, transparent);">
                                ...
                                <div class="entrega-option-card cursor-pointer p-4 rounded-lg border-2 transition-all duration-200 bg-dark-card hover:bg-dark-elevated <?php echo (($produto_edit['tipo_entrega'] ?? '') == 'manual') ? 'border-[var(--accent-primary)] bg-dark-elevated' : 'border-dark-border'; ?>" data-value="manual" onclick="if(typeof window.selectEntregaOption === 'function') { window.selectEntregaOption('manual'); } else { console.error('selectEntregaOption não está definida'); }" style="position: relative; z-index: 2; pointer-events: auto;">
                                    <div class="flex flex-col items-center text-center space-y-2">
                                        <div class="w-12 h-12 rounded-lg flex items-center justify-center mb-2" style="background-color: color-mix(in srgb, var(--accent-primary) 20%, transparent);">
                        ...
                        <div id="entrega-fields-container">
                            ...
                            <div id="entrega-fisico-container" class="animate-fade-in-down" style="display: <?php echo (($produto_edit['tipo_entrega'] ?? '') === 'manual') ? 'block' : 'none'; ?>;">
                    ...
                    <div class="bg-dark-card rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-dark-border flex flex-col overflow-hidden group">
                        ...
                        <div class="relative h-56 overflow-hidden bg-dark-elevated">
                            ...
                            <div class="absolute top-3 right-3 flex flex-col gap-2">
                                ...
                                <!-- Botão de Duplicar -->
                                <form method="post" action="/index?pagina=produtos">
                                    <?php
                                    if (!isset($csrf_token)) {
                                        $csrf_token = generate_csrf_token();
                                    }
                                    ...
                                    <button type="submit" name="duplicar_produto" class="bg-white/90 hover:bg-white text-gray-800 p-2 rounded-lg shadow-md transition-all duration-200 hover:shadow-lg backdrop-blur-sm w-full" title="Duplicar Produto">
