...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-white mb-6">CheckoutPRO Track</h1>
        ...
        <div class="flex items-center justify-between mb-6 border-b border-dark-border pb-4">
            <div>
                ...
                <p class="text-gray-400 text-sm mt-1">
                    <span id="realtime_indicator" class="inline-flex items-center">
                        <span class="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></span>
                        Atualizando em tempo real (a cada 5s)
                    </span>
                </p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const REALTIME_UPDATE_SECONDS = 5; // Atualizar a cada 5 segundos
    ...
    function startRealTimeUpdates() {
        stopRealTimeUpdates(); // Limpa intervalo anterior se existir
        realTimeInterval = setInterval(async () => {
            if (currentTrackingProductDbId) {
                await fetchTrackingData(currentTrackingProductDbId, currentPeriodFilter, true);
            }
        }, REALTIME_UPDATE_SECONDS * 1000);
        console.log('Atualização em tempo real iniciada (a cada ' + REALTIME_UPDATE_SECONDS + 's)');
    }
    ...
    async function fetchTrackingData(trackingProductDbId, period, silent = false) {
        try {
