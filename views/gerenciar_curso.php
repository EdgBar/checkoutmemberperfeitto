...
$aula_videos_dir = 'uploads/aula_videos/';

// Garante que os diretórios de arquivos de aula e vídeos existam
if (!is_dir($aula_files_dir)) {
    mkdir($aula_files_dir, 0755, true);
}
if (!is_dir($aula_videos_dir)) {
    mkdir($aula_videos_dir, 0755, true);
}

// Configuração do super admin: permitir ou não upload de vídeo direto na plataforma
$allow_video_upload = (function_exists('getSystemSetting') ? getSystemSetting('allow_video_upload', '0') : '0') === '1';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['adicionar_aula'])) {
            ...
            $video_source = $_POST['video_source'] ?? 'youtube'; // 'youtube' | 'upload'
            $url_video = trim($_POST['url_video'] ?? ''); // YouTube URL ou vazio se upload
            ...
            // Upload de vídeo MP4 (sem limite de tamanho na validação - usa 2GB como teto; aumente php.ini se precisar mais)
            if ($video_source === 'upload' && isset($_FILES['video_mp4']) && $_FILES['video_mp4']['error'] === UPLOAD_ERR_OK) {
                $allowed_video_types = ['video/mp4'];
                $allowed_video_ext = ['mp4'];
                $max_video_size = 2 * 1024 * 1024 * 1024; // 2GB
                $upload_result = validate_uploaded_file($_FILES['video_mp4'], $allowed_video_types, $allowed_video_ext, $max_video_size, $aula_videos_dir, 'aula_video');
                if ($upload_result['success']) {
                    $url_video = $upload_result['file_path']; // caminho relativo ex: uploads/aula_videos/xxx.mp4
                } else {
                    $mensagem = "<div class='bg-red-900/20 border border-red-500 text-red-300 px-4 py-3 rounded' role='alert'>" . htmlspecialchars($upload_result['error'] ?? 'Erro no upload do vídeo.') . "</div>";
                    $url_video = '';
                }
            }
            ...
            if ($stmt_check_modulo->rowCount() === 0) {
                ...
                if ($tipo_conteudo === 'video' && empty($url_video)) {
                    $mensagem = "<div class='bg-red-900/20 border border-red-500 text-red-300 px-4 py-3 rounded' role='alert'>Para aulas de vídeo, informe a URL do YouTube ou faça o upload de um vídeo MP4.</div>";
                    ...
                    $mensagem = "<div class='bg-red-900/20 border border-red-500 text-red-300 px-4 py-3 rounded' role='alert'>Para aulas mistas, o vídeo (URL ou upload) e pelo menos um arquivo são obrigatórios.</div>";
                } else {
        ...
        if (isset($_POST['editar_aula_form'])) {
            ...
            $titulo_aula = trim($_POST['titulo_aula']);
            $video_source_edit = $allow_video_upload ? ($_POST['video_source_edit'] ?? 'youtube') : 'youtube';
            $url_video = trim($_POST['url_video'] ?? '');
            $descricao_aula = trim($_POST['descricao_aula']);
            $release_days_aula = (int)($_POST['release_days_aula'] ?? 0);
            $tipo_conteudo = $_POST['tipo_conteudo'] ?? 'video';

            if ($video_source_edit === 'upload' && isset($_FILES['video_mp4_edit']) && $_FILES['video_mp4_edit']['error'] === UPLOAD_ERR_OK) {
                $allowed_video_types = ['video/mp4'];
                $allowed_video_ext = ['mp4'];
                // Sem limite de tamanho na plataforma (4GB; servidor pode limitar por php.ini)
                $max_video_size = 4 * 1024 * 1024 * 1024;
                $upload_result = validate_uploaded_file($_FILES['video_mp4_edit'], $allowed_video_types, $allowed_video_ext, $max_video_size, $aula_videos_dir, 'aula_video');
                if ($upload_result['success']) {
                    $url_video = $upload_result['file_path'];
                }
            }
    ...
    <div class="bg-dark-card rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] max-h-[90vh] transform transition-all opacity-0 scale-95 border border-[#32e768] flex flex-col my-4" id="add-lesson-modal-content">
        <form action="/index?pagina=gerenciar_curso&produto_id=<?php echo $produto_id; ?>" method="post" enctype="multipart/form-data" class="flex flex-col h-full min-h-0">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 min-h-0">
                ...
                <!-- Fonte do Vídeo (Add): YouTube sempre visível; Upload MP4 só se super admin permitir -->
                <div id="add-video-source-container">
                    <label class="block text-gray-300 text-sm font-semibold mb-2">Fonte do Vídeo</label>
                    <div class="flex gap-4 mb-3">
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="video_source" value="youtube" id="add_video_source_youtube" class="mr-2" checked>
                            <span class="text-gray-300">URL do YouTube</span>
                        </label>
                        <?php if ($allow_video_upload): ?>
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="video_source" value="upload" id="add_video_source_upload" class="mr-2">
                            <span class="text-gray-300">Upload MP4 na plataforma</span>
                        </label>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <?php if ($allow_video_upload): ?>
                <div id="add-video-upload-container" class="hidden">
                    <label for="add_video_mp4" class="block text-gray-300 text-sm font-semibold mb-2">Upload do Vídeo (MP4)</label>
                    <input type="file" id="add_video_mp4" name="video_mp4" accept="video/mp4,.mp4" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold" style="--file-bg: color-mix(in srgb, var(--accent-primary) 20%, transparent); --file-text: var(--accent-primary);">
                    <p class="mt-1 text-xs text-gray-400">Envie um vídeo em MP4. Sem limite de tamanho (o servidor pode ter restrição em php.ini).</p>
                </div>
                <?php endif; ?>
    ...
    <div class="bg-dark-card rounded-xl shadow-2xl w-full max-w-4xl h-[90vh] max-h-[90vh] transform transition-all opacity-0 scale-95 border border-[#32e768] flex flex-col my-4" id="edit-lesson-modal-content">
        <form action="/index?pagina=gerenciar_curso&produto_id=<?php echo $produto_id; ?>" method="post" enctype="multipart/form-data" class="flex flex-col h-full min-h-0">
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1 min-h-0">
                ...
                <!-- Fonte do Vídeo (Edit): YouTube sempre visível; Upload MP4 só se super admin permitir -->
                <div id="edit-video-source-container">
                    <label class="block text-gray-300 text-sm font-semibold mb-2">Fonte do Vídeo</label>
                    <div class="flex gap-4 mb-3">
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="video_source_edit" value="youtube" id="edit_video_source_youtube" class="mr-2" checked>
                            <span class="text-gray-300">URL do YouTube</span>
                        </label>
                        <?php if ($allow_video_upload): ?>
                        <label class="flex items-center cursor-pointer">
                            <input type="radio" name="video_source_edit" value="upload" id="edit_video_source_upload" class="mr-2">
                            <span class="text-gray-300">Upload MP4 na plataforma</span>
                        </label>
                        <?php endif; ?>
                    </div>
                ...
                <?php if ($allow_video_upload): ?>
                <div id="edit-video-upload-container" class="hidden">
                    <label for="edit_video_mp4" class="block text-gray-300 text-sm font-semibold mb-2">Substituir por novo vídeo (MP4)</label>
                    <input type="file" id="edit_video_mp4" name="video_mp4_edit" accept="video/mp4,.mp4" class="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold" style="--file-bg: color-mix(in srgb, var(--accent-primary) 20%, transparent); --file-text: var(--accent-primary);">
                    <p class="mt-1 text-xs text-gray-400">Deixe em branco para manter o vídeo atual.</p>
                </div>
                <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const addVideoSourceYouTube = document.getElementById('add_video_source_youtube');
    const addVideoSourceUpload = document.getElementById('add_video_source_upload');
    const addVideoUploadContainer = document.getElementById('add-video-upload-container');
    const addVideoMp4Input = document.getElementById('add_video_mp4');

    function toggleAddLessonFields() {
        const selectedType = addTipoConteudoSelect.value;
        const sourceYouTube = addVideoSourceYouTube && addVideoSourceYouTube.checked;
        addUrlVideoInput.required = false;
        if (addVideoMp4Input) addVideoMp4Input.required = false;
        addAulaFilesInput.required = false;

        addVideoUrlContainer.style.display = 'none';
        if (addVideoUploadContainer) addVideoUploadContainer.classList.add('hidden');
        ...
        if (selectedType === 'video' || selectedType === 'mixed') {
            ...
            } else if (addVideoUploadContainer && addVideoMp4Input) {
                addVideoUploadContainer.classList.remove('hidden');
                addVideoMp4Input.required = true;
            }
        }
    ...
    if (addVideoSourceYouTube) addVideoSourceYouTube.addEventListener('change', toggleAddLessonFields);
    if (addVideoSourceUpload) addVideoSourceUpload.addEventListener('change', toggleAddLessonFields);
    ...
    const editVideoSourceYouTube = document.getElementById('edit_video_source_youtube');
    const editVideoSourceUpload = document.getElementById('edit_video_source_upload');
    const editVideoUploadContainer = document.getElementById('edit-video-upload-container');
    ...
    function toggleEditLessonFields() {
        ...
        const hasUploadOption = editVideoSourceUpload != null;
        const sourceYouTube = editVideoSourceYouTube ? editVideoSourceYouTube.checked : true;
        ...
        if (editVideoUploadContainer) editVideoUploadContainer.classList.add('hidden');
        editExistingFilesContainer.style.display = 'none';
        editNewFilesUploadContainer.style.display = 'none';

        if (selectedType === 'video' || selectedType === 'mixed') {
            if (sourceYouTube || !hasUploadOption) {
                editVideoUrlContainer.style.display = 'block';
                editUrlVideoInput.required = true;
            } else if (editVideoUploadContainer) {
                editVideoUploadContainer.classList.remove('hidden');
            }
        }
        if (selectedType === 'files' || selectedType === 'mixed') {
    ...
    if (editVideoSourceYouTube) editVideoSourceYouTube.addEventListener('change', toggleEditLessonFields);
    if (editVideoSourceUpload) editVideoSourceUpload.addEventListener('change', toggleEditLessonFields);
    ...
    document.querySelectorAll('.edit-lesson-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Selecionar YouTube ou Upload conforme o vídeo da aula
            const isUploadVideo = urlVideo && (urlVideo.indexOf('uploads/') === 0 || urlVideo.toLowerCase().endsWith('.mp4'));
            if (editVideoSourceUpload && editVideoSourceYouTube) {
                if (isUploadVideo) {
                    editVideoSourceUpload.checked = true;
                    editVideoSourceYouTube.checked = false;
                } else {
                    editVideoSourceYouTube.checked = true;
                    editVideoSourceUpload.checked = false;
                }
            }
