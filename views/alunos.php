...
<div class="container mx-auto relative">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center gap-2">
            <button id="btn-bulk-alunos" class="hidden px-4 py-2 rounded-lg font-semibold text-white transition-all duration-300 flex items-center gap-2 bg-dark-elevated border border-dark-border hover:bg-dark-card">
...
<!-- MODAL: INSERIR ALUNOS EM MASSA -->
<div id="bulk-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="bulk-modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-900 bg-opacity-75 transition-opacity" aria-hidden="true"></div>
        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
        <div class="inline-block align-bottom bg-dark-card rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border" style="border-color: var(--accent-primary);">
            <div class="bg-dark-card px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10" style="background-color: color-mix(in srgb, var(--accent-primary) 20%, transparent);">
                        <i data-lucide="users" class="h-6 w-6" style="color: var(--accent-primary);"></i>
                    </div>
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        <h3 class="text-lg leading-6 font-medium text-white" id="bulk-modal-title">Inserir Alunos em Massa</h3>
                        <p class="mt-2 text-sm text-gray-400">
                            Cole uma lista com <strong>uma linha por aluno</strong>, no formato: <code class="text-gray-300">email,nome</code>
                        </p>

                        <div class="mt-4 space-y-4">
                            <div>
                                <label for="bulk-produto" class="block text-sm font-medium text-gray-300">Produto *</label>
                                <select id="bulk-produto" class="mt-1 block w-full shadow-sm sm:text-sm border-dark-border rounded-md border p-2 bg-dark-elevated text-white" required>
                                    <option value="">-- Selecione --</option>
                                    <?php foreach ($produtos_area_membros as $produto): ?>
                                        <option value="<?php echo $produto['id']; ?>"><?php echo htmlspecialchars($produto['nome']); ?></option>
                                    <?php endforeach; ?>
                            ...
                            <div>
                                <label for="bulk-lista" class="block text-sm font-medium text-gray-300">Lista (email,nome) *</label>
                                <textarea id="bulk-lista" rows="8" class="mt-1 block w-full shadow-sm sm:text-sm border-dark-border rounded-md border p-2 bg-dark-elevated text-white placeholder-gray-500" placeholder="weverton@gmail.com,Weverton de Paula&#10;ana@gmail.com,Ana Lúcia"></textarea>
                                <p class="mt-2 text-xs text-gray-400">
                                    Dica: a primeira vírgula separa o e-mail do nome. Linhas vazias serão ignoradas.
                                </p>
                            </div>

                            <div class="flex items-center">
                                <input type="checkbox" id="bulk-enviar-email" class="w-4 h-4 rounded border-dark-border bg-dark-elevated text-primary focus:ring-primary" checked>
                                <label for="bulk-enviar-email" class="ml-2 text-sm text-gray-300">Enviar email de acesso automaticamente para todos</label>
                            </div>

                            <div id="bulk-result" class="hidden mt-2 p-3 rounded-lg border border-dark-border bg-dark-elevated text-sm text-gray-300"></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-dark-elevated px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" id="confirm-bulk-btn" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm" style="background-color: var(--accent-primary);" onmouseover="this.style.backgroundColor='var(--accent-primary-hover)'" onmouseout="this.style.backgroundColor='var(--accent-primary)'">
                    Inserir Alunos
                </button>
                <button type="button" id="cancel-bulk-btn" class="mt-3 w-full inline-flex justify-center rounded-md border border-dark-border shadow-sm px-4 py-2 bg-dark-card text-base font-medium text-gray-300 hover:bg-dark-elevated focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                    Cancelar
                </button>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const btnBulkAlunos = document.getElementById('btn-bulk-alunos');
    ...
    // Abrir modal de inserção em massa
    if (btnBulkAlunos) {
        btnBulkAlunos.addEventListener('click', function() {
            const bulkProduto = document.getElementById('bulk-produto');
            const bulkLista = document.getElementById('bulk-lista');
            const bulkResult = document.getElementById('bulk-result');
            if (bulkProduto) bulkProduto.value = currentProdutoId || '';
            if (bulkLista) bulkLista.value = '';
            if (bulkResult) { bulkResult.classList.add('hidden'); bulkResult.textContent = ''; }
    ...
    // Inserir alunos em massa
    if (confirmBulkBtn) {
        confirmBulkBtn.addEventListener('click', async function() {
            const produtoId = document.getElementById('bulk-produto')?.value;
            const lista = document.getElementById('bulk-lista')?.value || '';
            const enviarEmail = document.getElementById('bulk-enviar-email')?.checked === true;
            const bulkResult = document.getElementById('bulk-result');
            ...
            if (!lista.trim()) {
                alert('Cole a lista no formato email,nome (uma linha por aluno).');
                return;
            }

            confirmBulkBtn.disabled = true;
            confirmBulkBtn.textContent = 'Processando...';
            if (bulkResult) {
                bulkResult.classList.remove('hidden');
                bulkResult.textContent = 'Processando...';
            }

            try {
                const response = await fetch('/api/alunos_api.php?action=bulk_create', {
                    method: 'POST',
                ...
                let resumo = `Concluído.\n\n` +
                    `Produto: ${r.produto_nome || produtoId}\n` +
                    `Linhas processadas: ${r.total_linhas || 0}\n` +
                    `Acessos criados: ${r.criados || 0}\n` +
                    `Já tinham acesso: ${r.ja_tinham_acesso || 0}\n` +
                    `Emails enviados: ${r.emails_enviados || 0}\n`;

                if (Array.isArray(r.erros) && r.erros.length > 0) {
                    resumo += `\nErros (${r.erros.length}):\n` + r.erros.slice(0, 10).map(e => `Linha ${e.linha}: ${e.erro}`).join('\n');
                    if (r.erros.length > 10) resumo += `\n... e mais ${r.erros.length - 10} erro(s)`;
                }

                if (bulkResult) bulkResult.textContent = resumo;
                alert(resumo);

                // Atualiza listagem se estiver no mesmo produto
                if (String(currentProdutoId) === String(produtoId)) {
                    fetchAlunos();
                }
            } catch (err) {
                console.error(err);
                const msg = err.message || 'Erro ao inserir alunos em massa';
                if (bulkResult) {
                    bulkResult.classList.remove('hidden');
                ...
                confirmBulkBtn.textContent = 'Inserir Alunos';
            }
        });
    }
