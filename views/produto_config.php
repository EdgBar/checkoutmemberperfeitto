...
if (isset($_POST['salvar_produto_config'])) {
    ...
    if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
        ...
        $normalize_tipo_entrega = function ($t) {
        ...
        // Robustez: se o PDF foi enviado, força tipo_entrega = arquivo
        ...
        $tipo_entrega = $normalize_tipo_entrega($tipo_entrega);
        ...
        if ($tipo_entrega === 'link_externo') {
            ...
            if (isset($_FILES['conteudo_entrega_pdf']) && is_array($_FILES['conteudo_entrega_pdf'])) {
                ...
                if ($pdf_error === UPLOAD_ERR_OK) {
                    ...
                    if ($upload_result['success']) {
                        ...
                        // Em caso de erro, mantém o PDF atual e mostra mensagem
                        $conteudo_entrega = $conteudo_entrega_atual;
                        $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Falha ao enviar o PDF: " . htmlspecialchars($upload_result['error']) . "</div>";
                    }
                } elseif ($pdf_error !== UPLOAD_ERR_NO_FILE) {
                    // Erros de upload (tamanho/servidor/permissão/temp dir etc.)
                    $error_messages = [
                    ...
                    $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Falha ao enviar o PDF: " . htmlspecialchars($error_messages[$pdf_error] ?? ('Erro no upload (código ' . (int)$pdf_error . ')')) . "</div>";
                }
            }
            ...
            if (empty($conteudo_entrega)) {
                $entrega_valid = false;
                $mensagem = "<div class='bg-red-900/20 border-l-4 border-red-500 text-red-300 p-4 rounded-md shadow-sm mb-6' role='alert'>Você selecionou <strong>Arquivo PDF</strong> como entrega, mas nenhum PDF foi enviado/salvo. Envie um arquivo PDF (até 10MB) e salve novamente.</div>";
            }
        }
        ...
        // Sincronizar nome e descrição do produto com o curso (quando entrega = área de membros), para exibição consistente na área de membros
        if ($tipo_entrega === 'area_membros') {
            $stmt_curso = $pdo->prepare("UPDATE cursos SET titulo = ?, descricao = ? WHERE produto_id = ?");
            $stmt_curso->execute([$nome, $descricao ?? '', $id_produto]);
        }
        ...
        $has_tracking_fields = (isset($_POST['facebookPixelId']) && $_POST['facebookPixelId'] !== '') || 
                               ...
                               (isset($_POST['customScriptB64']) && $_POST['customScriptB64'] !== '') ||
        ...
        if ($has_tracking_fields) {
            ...
            // Decodifica o customScript de base64 (enviado assim para evitar bloqueio do WAF)
            $customScript_decoded = '';
            if (isset($_POST['customScriptB64']) && $_POST['customScriptB64'] !== '') {
                $customScript_decoded = base64_decode($_POST['customScriptB64']);
                if ($customScript_decoded === false) {
                    $customScript_decoded = $existing_tracking['customScript'] ?? '';
                }
            } else {
                $customScript_decoded = $existing_tracking['customScript'] ?? '';
            }
            ...
            $config_array['tracking'] = [
                ...
                'customScript' => $customScript_decoded,
                'events' => [
                    'facebook' => [
        ...
        $has_payment_methods_in_post = isset($_POST['payment_pix_pushinpay']) || isset($_POST['payment_pix_efi']) || isset($_POST['payment_pix_enabled']) || 
                                       isset($_POST['payment_credit_card_mercadopago']) || isset($_POST['payment_credit_card_hypercash']) || isset($_POST['payment_credit_card_efi']) || isset($_POST['payment_credit_card_stripe']) || 
        ...
        if ($has_payment_methods_in_post) {
            ...
            // Determinar gateway do Cartão (prioridade: Stripe > Hypercash > Efí > Mercado Pago)
        ...
        $lista_produtos_orderbump = $stmt_todos_produtos->fetchAll(PDO::FETCH_ASSOC);
            ...
            $pdo->rollBack();
            // Preserva mensagens mais específicas (ex.: falha no upload/validação do PDF)
            if (empty($mensagem)) {
