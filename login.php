...
// Buscar configurações de textos do login
$login_config = [];
$login_defaults = [
...
try {
    $keys = array_keys($login_defaults);
    $placeholders = implode(',', array_fill(0, count($keys), '?'));
    $stmt = $pdo->prepare("SELECT chave, valor FROM configuracoes WHERE chave IN ($placeholders)");
    $stmt->execute($keys);
    $results = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
    
    foreach ($login_defaults as $key => $default) {
        $login_config[$key] = $results[$key] ?? $default;
    }
} catch (PDOException $e) {
    $login_config = $login_defaults;
}

// Buscar URL de fundo do login (imagem ou vídeo)
$login_bg_url = '';
try {
    // Buscar na tabela configuracoes_sistema (usada pelo setSystemSetting)
    $stmt = $pdo->prepare("SELECT valor FROM configuracoes_sistema WHERE chave = 'login_bg_url' LIMIT 1");
    $stmt->execute();
    $result = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($result && !empty($result['valor'])) {
        $login_bg_url = $result['valor'];
    }
} catch (PDOException $e) {
    // Ignora erro
}
...
$is_video_bg = !empty($login_bg_url) && preg_match('/\.(mp4|webm)$/i', $login_bg_url);
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body class="min-h-screen" style="background-color: #07090d;">
    ...
    <div class="min-h-screen grid lg:grid-cols-2">
        ...
        <div class="hidden lg:flex relative flex-col justify-end p-12 overflow-hidden bg-slate-900">
            <?php if (!empty($login_bg_url)): ?>
                <?php if ($is_video_bg): ?>
                <!-- Vídeo de Fundo -->
                <div class="absolute inset-0 z-0">
                    <video autoplay muted loop playsinline class="w-full h-full object-cover">
                        <source src="<?php echo htmlspecialchars($login_bg_url); ?>" type="video/<?php echo pathinfo($login_bg_url, PATHINFO_EXTENSION); ?>">
                    </video>
                ...
                <div class="absolute inset-0 z-0">
                    <img src="<?php echo htmlspecialchars($login_bg_url); ?>" 
            ...
            <div class="absolute top-8 left-8 z-30">
                <span class="px-4 py-2 rounded-full text-sm font-semibold" style="background: <?php echo htmlspecialchars($login_config['login_badge_color']); ?>20; color: <?php echo htmlspecialchars($login_config['login_badge_color']); ?>; border: 1px solid <?php echo htmlspecialchars($login_config['login_badge_color']); ?>40;">
                    <?php echo htmlspecialchars($login_config['login_badge_text']); ?>
                </span>
            </div>

            <div class="relative z-20 mb-8 max-w-lg">
                <!-- Título Principal -->
                <h1 class="text-5xl font-bold mb-4 leading-tight" style="color: <?php echo htmlspecialchars($login_config['login_title_color']); ?>;">
                    <?php echo htmlspecialchars($login_config['login_title_line1']); ?> <br>
                    <span class="text-transparent bg-clip-text" style="background-image: linear-gradient(to right, <?php echo htmlspecialchars($login_config['login_title_highlight_color1']); ?>, <?php echo htmlspecialchars($login_config['login_title_highlight_color2']); ?>);">
                        <?php echo htmlspecialchars($login_config['login_title_line2']); ?>
                    </span>
                </h1>
                
                <!-- Descrição -->
                <p class="text-lg leading-relaxed mb-8" style="color: <?php echo htmlspecialchars($login_config['login_description_color']); ?>;">
                    <?php echo htmlspecialchars($login_config['login_description']); ?>
                </p>
                
                <!-- Estatísticas Flutuantes -->
                <div class="flex gap-8 mt-6">
                    <div>
                        <p class="text-2xl font-bold" style="color: <?php echo htmlspecialchars($login_config['login_stats_value_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat1_value']); ?></p>
                        <p class="text-sm" style="color: <?php echo htmlspecialchars($login_config['login_stats_label_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat1_label']); ?></p>
                    </div>
                    <div>
                        <p class="text-2xl font-bold" style="color: <?php echo htmlspecialchars($login_config['login_stats_value_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat2_value']); ?></p>
                        <p class="text-sm" style="color: <?php echo htmlspecialchars($login_config['login_stats_label_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat2_label']); ?></p>
                    </div>
                    <div>
                        <p class="text-2xl font-bold" style="color: <?php echo htmlspecialchars($login_config['login_stats_value_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat3_value']); ?></p>
                        <p class="text-sm" style="color: <?php echo htmlspecialchars($login_config['login_stats_label_color']); ?>;"><?php echo htmlspecialchars($login_config['login_stat3_label']); ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-center p-8" style="background-color: #07090d;">
            <div class="w-full max-w-[420px] space-y-8">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="space-y-6">
                    ...
                    <button type="submit" class="w-full text-white font-bold py-4 px-6 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2 group" style="background: linear-gradient(to right, <?php echo htmlspecialchars($login_config['login_btn_color1']); ?>, <?php echo htmlspecialchars($login_config['login_btn_color2']); ?>, <?php echo htmlspecialchars($login_config['login_btn_color3']); ?>); box-shadow: 0 10px 15px -3px <?php echo htmlspecialchars($login_config['login_btn_color1']); ?>40, 0 4px 6px -2px <?php echo htmlspecialchars($login_config['login_btn_color2']); ?>30;">
