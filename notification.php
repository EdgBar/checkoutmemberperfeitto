...
// Sessão já é iniciada no config.php, não precisa iniciar novamente
// Apenas garante que está ativa
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
error_log("NOTIFICATION INIT: Session status after config: " . session_status() . ", Session ID: " . session_id());
...
function trigger_webhooks($usuario_id, $event_data, $trigger_event, $produto_id = null) {
    ...
    try {
        ...
        foreach ($webhooks as $url) {
            ...
            $ch = curl_init($url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json', 'X-CheckoutPRO-Event: ' . $trigger_event]);
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    if (in_array($type, ['link', 'link_externo'], true) && !empty($content)) return ['success' => true, 'product_name' => $product_data['produto_nome'], 'content_type' => 'link', 'content_value' => $content];
    if (in_array($type, ['email_pdf', 'arquivo'], true) && !empty($content)) {
        $safe_content = basename((string)$content);
        $pdf_path = __DIR__ . '/uploads/' . $safe_content;
        if (!empty($safe_content) && file_exists($pdf_path) && is_readable($pdf_path)) {
    ...
    if (in_array($type, ['produto_fisico', 'manual'], true)) {
...
function send_delivery_email_consolidated($to_email, $customer_name, $products, $pass, $login_url, $address_data = null, $setup_token = null) {
    ...
    try {
        ...
        $mail->setFrom($fromEmail, $config['smtp_from_name'] ?? 'CheckoutPRO');
        $mail->addAddress($to_email, $customer_name);
        
        // Usa o assunto do banco ou um padrão (verifica se está vazio também)
        $subject_from_db = $config['email_template_delivery_subject'] ?? '';
        $mail->Subject = !empty(trim($subject_from_db)) ? $subject_from_db : 'Seu acesso chegou!';
        $mail->isHTML(true);
        $mail->CharSet = 'UTF-8';
...
function create_notification($usuario_id, $tipo, $mensagem, $valor, $venda_id_fk = null, $metodo = null) {
    ...
    log_webhook("CREATE_NOTIFICATION chamada - usuario_id: $usuario_id, tipo: $tipo, mensagem: $mensagem, valor: $valor, venda_id: $venda_id_fk, metodo: $metodo");
    
    if (!$usuario_id) {
        log_webhook("CREATE_NOTIFICATION: usuario_id vazio, abortando");
        return;
    }
    ...
    try {
        ...
        $stmt->execute([$usuario_id, $tipo, $mensagem, $valor, $link, $venda_id_fk, $metodo]);
        log_webhook("CREATE_NOTIFICATION: Notificação criada com sucesso! ID: " . $pdo->lastInsertId());
    } catch (Exception $e) {
        log_webhook("CREATE_NOTIFICATION ERRO: " . $e->getMessage());
        log_webhook("CREATE_NOTIFICATION ERRO SQL: " . print_r($stmt->errorInfo(), true));
    }
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Formato Stripe - payment_intent.succeeded (id em data.object.id)
        if (isset($data['type']) && $data['type'] === 'payment_intent.succeeded' && isset($data['data']['object']['id'])) {
            $payment_id = $data['data']['object']['id'];
            $gateway_detected = 'stripe';
            log_webhook("Payment ID extraído do formato Stripe (payment_intent.succeeded): " . $payment_id);
        }
        
        // Formato Efí Cartão - charge_id (prioridade para detectar gateway de cartão)
        if (!$payment_id && isset($data['data']['charge_id'])) {
        ...
        } elseif (!$payment_id && isset($data['charge_id'])) {
        ...
        if ($payment_id) {
            ...
            if ($venda) {
                ...
                // Formato Stripe - payment_intent.succeeded já indica aprovação
                if (!$new_status && $gateway_detected === 'stripe') {
                    if (isset($data['type']) && $data['type'] === 'payment_intent.succeeded') {
                        $new_status = 'approved';
                        log_webhook("Status Stripe (payment_intent.succeeded): approved");
                    } elseif (isset($data['data']['object']['status'])) {
                        $st = strtolower($data['data']['object']['status']);
                        if ($st === 'succeeded') $new_status = 'approved';
                        elseif ($st === 'requires_payment_method' || $st === 'canceled') $new_status = 'rejected';
                        else $new_status = 'pending';
                        log_webhook("Status Stripe (object.status): " . $new_status);
                    }
                }
                ...
                if ($should_process) {
                    ...
                    if (!empty($all_sales)) {
                        ...
                        // CORREÇÃO: Log detalhado antes de criar notificação
                        log_webhook("NOTIFICACAO: Preparando criar notificação - usuario_id: " . ($main_sale['usuario_id'] ?? 'NULL') . " | tipo: " . ($db_status == 'approved' ? 'Compra Aprovada' : 'Atualização') . " | msg: $msg | valor: " . ($webhook_payload['valor_total_compra'] ?? 0) . " | venda_id: " . ($main_sale['id'] ?? 'NULL'));
                        ...
                        log_webhook("NOTIFICACAO: Função create_notification executada");
                        ...
                        if (plugin_active('saas')) {
                            ...
                            if ($assinatura_plano) {
                                if ($db_status === 'approved' || $db_status === 'paid') {
                                    // IMPORTANTE: Desativar TODAS as outras assinaturas do usuário (ativas, trial, etc) antes de ativar a nova
                                    ...
                                    $canceladas_webhook = $stmt_desativar->rowCount();
                                    log_webhook("SaaS (Webhook): Assinaturas anteriores suspensas: $canceladas_webhook");
                                    ...
                                    $has_notificado_vencimento = in_array('notificado_vencimento', $colunas);
                                    $has_notificado_expirado = in_array('notificado_expirado', $colunas);
                                    ...
                                    if ($has_data_vencimento) {
                                        $update_fields[] = 'data_vencimento = ?';
                                        $update_values[] = $novo_vencimento;
                                    }
                                    
                                    if ($has_notificado_vencimento) {
                                        $update_fields[] = 'notificado_vencimento = 0';
                                    }
                                    
                                    if ($has_notificado_expirado) {
                                        $update_fields[] = 'notificado_expirado = 0';
                                    }
                                    
                                    $update_values[] = $payment_id;
                                    
                                    $sql = "UPDATE saas_assinaturas SET " . implode(', ', $update_fields) . " WHERE transacao_id = ?";
                                    $stmt_update = $pdo->prepare($sql);
                                    $stmt_update->execute($update_values);
                                    
                                    log_webhook("SaaS (Webhook): Assinatura atualizada - transacao_id: $payment_id, status: ativo, vencimento: $novo_vencimento");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        error_log("NOTIFICATION API: Session status: " . session_status());
        error_log("NOTIFICATION API: Session ID: " . session_id());
        error_log("NOTIFICATION API: SESSION data: " . json_encode($_SESSION));
        
        if (!isset($_SESSION['id'])) { 
            http_response_code(401);
            error_log("NOTIFICATION API: Auth failed - SESSION[id] not set");
            echo json_encode(['error' => 'Auth required', 'debug' => 'session_id_missing']); 
            exit; 
        }
        
        $uid = $_SESSION['id'];
        error_log("NOTIFICATION API: Auth OK - uid=$uid");
        $action = $_GET['action'] ?? '';
