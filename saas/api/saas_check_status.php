...
try {
    // Buscar assinatura
    error_log("SaaS Check Status: Buscando assinatura com transacao_id: $payment_id, gateway: $gateway");
    $stmt = $pdo->prepare("SELECT * FROM saas_assinaturas WHERE transacao_id = ? LIMIT 1");
    $stmt->execute([$payment_id]);
    $assinatura = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$assinatura) {
        error_log("SaaS Check Status: Assinatura não encontrada para transacao_id: $payment_id");
        returnJsonError('Assinatura não encontrada.', 404);
    }
    
    error_log("SaaS Check Status: Assinatura encontrada - ID: {$assinatura['id']}, Status atual: {$assinatura['status']}, Usuário: {$assinatura['usuario_id']}");
    ...
    if ($gateway === 'efi') {
        ...
        if ($status_data && isset($status_data['status'])) {
            ...
            error_log("SaaS Check Status (Efí): Status do gateway: {$status_data['status']}, Normalizado: $status, Status atual da assinatura: {$assinatura['status']}");
            
            if ($status === 'approved' && $assinatura['status'] !== 'ativa') {
                error_log("SaaS Check Status (Efí): Ativando assinatura #{$assinatura['id']}");
                ...
                ");
                $stmt_cancel->execute([$assinatura['usuario_id'], $assinatura['id']]);
                $canceladas = $stmt_cancel->rowCount();
                error_log("SaaS Check Status (Efí): Assinaturas anteriores suspensas: $canceladas");
                ...
                // Montar query dinamicamente baseado nas colunas disponíveis
                ...
                $update_values[] = $assinatura['id'];
                
                $sql = "UPDATE saas_assinaturas SET " . implode(', ', $update_fields) . " WHERE id = ?";
                $stmt_update = $pdo->prepare($sql);
                $stmt_update->execute($update_values);
                
                // Verificar se a atualização foi bem-sucedida
                $rows_updated = $stmt_update->rowCount();
                error_log("SaaS (Efí): Assinatura #{$assinatura['id']} ativada para usuário #{$assinatura['usuario_id']}. Vencimento: {$novo_vencimento}. Linhas atualizadas: $rows_updated");
                
                // Verificar se o plano está realmente ativo agora
                $stmt_verify = $pdo->prepare("SELECT id, status, plano_id FROM saas_assinaturas WHERE id = ?");
                $stmt_verify->execute([$assinatura['id']]);
                $verify = $stmt_verify->fetch(PDO::FETCH_ASSOC);
                if ($verify) {
                    error_log("SaaS (Efí): Verificação pós-ativação - Status: {$verify['status']}, Plano ID: {$verify['plano_id']}");
                }
            }
        ...
        if ($data && isset($data['status'])) {
            ...
            error_log("SaaS Check Status (PushinPay): Status do gateway: $status_raw, Normalizado: $status, Status atual da assinatura: {$assinatura['status']}");
            
            if ($status === 'approved' && $assinatura['status'] !== 'ativa') {
                error_log("SaaS Check Status (PushinPay): Ativando assinatura #{$assinatura['id']} para usuário #{$assinatura['usuario_id']}");
                // Buscar informações do plano para calcular vencimento
                $stmt_plano = $pdo->prepare("SELECT periodo FROM saas_planos WHERE id = ?");
                $stmt_plano->execute([$assinatura['plano_id']]);
                $plano = $stmt_plano->fetch(PDO::FETCH_ASSOC);
                ...
                $stmt_cancel->execute([$assinatura['usuario_id'], $assinatura['id']]);
                $canceladas = $stmt_cancel->rowCount();
                error_log("SaaS Check Status (PushinPay): Assinaturas anteriores suspensas: $canceladas");
                ...
                // Verificar se a atualização foi bem-sucedida
                $rows_updated = $stmt_update->rowCount();
                error_log("SaaS (PushinPay): Assinatura #{$assinatura['id']} ativada para usuário #{$assinatura['usuario_id']}. Vencimento: {$novo_vencimento}. Linhas atualizadas: $rows_updated");
                ...
                $verify = $stmt_verify->fetch(PDO::FETCH_ASSOC);
                if ($verify) {
                    error_log("SaaS (PushinPay): Verificação pós-ativação - Status: {$verify['status']}, Plano ID: {$verify['plano_id']}");
                }
            }
        ...
        if ($http_code == 200) {
            ...
            if (isset($data['status'])) {
                ...
                error_log("SaaS Check Status (Mercado Pago): Status do gateway: $status, Normalizado: $normalized_status, Status atual da assinatura: {$assinatura['status']}");
                
                if ($normalized_status === 'approved' && $assinatura['status'] !== 'ativa') {
                    error_log("SaaS Check Status (Mercado Pago): Ativando assinatura #{$assinatura['id']} para usuário #{$assinatura['usuario_id']}");
                    // Buscar informações do plano para calcular vencimento
                    $stmt_plano = $pdo->prepare("SELECT periodo FROM saas_planos WHERE id = ?");
                    $stmt_plano->execute([$assinatura['plano_id']]);
                    $plano = $stmt_plano->fetch(PDO::FETCH_ASSOC);
                    ...
                    $stmt_cancel->execute([$assinatura['usuario_id'], $assinatura['id']]);
                    $canceladas = $stmt_cancel->rowCount();
                    error_log("SaaS Check Status (Mercado Pago): Assinaturas anteriores suspensas: $canceladas");
                    ...
                    // Verificar se a atualização foi bem-sucedida
                    $rows_updated = $stmt_update->rowCount();
                    error_log("SaaS (Mercado Pago): Assinatura #{$assinatura['id']} ativada para usuário #{$assinatura['usuario_id']}. Vencimento: {$novo_vencimento}. Linhas atualizadas: $rows_updated");
                    ...
                    $verify = $stmt_verify->fetch(PDO::FETCH_ASSOC);
                    if ($verify) {
                        error_log("SaaS (Mercado Pago): Verificação pós-ativação - Status: {$verify['status']}, Plano ID: {$verify['plano_id']}");
                    }
                }
