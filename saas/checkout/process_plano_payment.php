...
if (file_exists(__DIR__ . '/../includes/saas_functions.php')) {
    ...
    // Garantir estrutura da tabela antes de qualquer operação
    ...
    try {
        // A tabela saas_config usa sistema de chave-valor, não coluna enabled
        $stmt_saas = $pdo->prepare("SELECT valor FROM saas_config WHERE chave = 'enabled' LIMIT 1");
        $stmt_saas->execute();
        $saas_config = $stmt_saas->fetch(PDO::FETCH_ASSOC);
        
        if (!$saas_config) {
            error_log("SaaS: Nenhum registro encontrado na tabela saas_config para chave 'enabled'");
        ...
        $enabled_value = (int)($saas_config['valor'] ?? 0);
...
try {
    ...
    if ($gateway_choice === 'efi' && $data['payment_method'] === 'pix') {
        ...
        $status = 'trial'; // Status inicial antes do pagamento ser aprovado
        
        // Garantir estrutura da tabela antes de inserir
        if (function_exists('saas_ensure_table_structure')) {
            saas_ensure_table_structure();
        }
        ...
        $has_transacao_id = in_array('transacao_id', $colunas);
        $has_metodo_pagamento = in_array('metodo_pagamento', $colunas);
        
        // Montar query dinamicamente baseado nas colunas disponíveis
        $campos = ['usuario_id', 'plano_id', 'status', 'data_inicio'];
        $valores = [$usuario_id, $plano_id, $status, $data_inicio];
        $placeholders = ['?', '?', '?', '?'];
        ...
        if ($has_metodo_pagamento) {
            $campos[] = 'metodo_pagamento';
            $valores[] = 'Pix';
            $placeholders[] = '?';
        }
        
        $sql = "INSERT INTO saas_assinaturas (" . implode(', ', $campos) . ") VALUES (" . implode(', ', $placeholders) . ")";
