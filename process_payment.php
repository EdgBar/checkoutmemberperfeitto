...
// Inclui o helper de notificações
$notification_helper_path = __DIR__ . '/helpers/notification_helper.php';
...
// --- Checkout gratuito (produto R$ 0,00): libera acesso sem cartão/Pix ---
if (!empty($data['free_checkout']) && !empty($data['checkout_hash'])) {
    $checkout_hash = trim($data['checkout_hash']);
    $stmt_prod = $pdo->prepare("SELECT id, usuario_id, nome, preco, tipo_entrega FROM produtos WHERE checkout_hash = ? LIMIT 1");
    $stmt_prod->execute([$checkout_hash]);
    $prod = $stmt_prod->fetch(PDO::FETCH_ASSOC);
    if (!$prod || (float)$prod['preco'] != 0) {
        returnJsonError('Produto não encontrado ou não é gratuito.', 400);
    }
    $email = filter_var($data['email'] ?? '', FILTER_SANITIZE_EMAIL);
    $name = sanitize_input($data['name'] ?? '');
    $cpf = preg_replace('/[^0-9]/', '', $data['cpf'] ?? '');
    $phone = preg_replace('/[^0-9]/', '', $data['phone'] ?? '');
    if (!validate_email($email)) { returnJsonError('Email inválido.', 400); }
    if (!validate_cpf($cpf)) { returnJsonError('CPF inválido.', 400); }
    if (!validate_phone_br($phone)) { returnJsonError('Telefone inválido.', 400); }
    if (empty($name)) { returnJsonError('Nome obrigatório.', 400); }
    $csrf = $data['csrf_token'] ?? '';
    if (empty($csrf) || !verify_csrf_token($csrf)) {
        returnJsonError('Token CSRF inválido.', 403);
    }
    $uuid = uniqid('checkout_') . bin2hex(random_bytes(8));
    $transacao_id = 'gratuito_' . bin2hex(random_bytes(8));
    try {
        $pdo->beginTransaction();
        $stmt_v = $pdo->prepare("INSERT INTO vendas (produto_id, comprador_nome, comprador_email, comprador_cpf, comprador_telefone, comprador_cep, comprador_logradouro, comprador_numero, comprador_complemento, comprador_bairro, comprador_cidade, comprador_estado, valor, status_pagamento, transacao_id, metodo_pagamento, checkout_session_uuid, email_entrega_enviado, utm_source, utm_campaign, utm_medium, utm_content, utm_term, src, sck) VALUES (?, ?, ?, ?, ?, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 0, 'approved', ?, 'gratuito', ?, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL)");
        $stmt_v->execute([$prod['id'], $name, $email, $cpf, $phone, $transacao_id, $uuid]);
        $pdo->prepare("INSERT IGNORE INTO alunos_acessos (aluno_email, produto_id) VALUES (?, ?)")->execute([$email, $prod['id']]);
        $pdo->commit();
        $redirect = '/obrigado?payment_id=' . urlencode($transacao_id);
        returnJsonSuccess(['success' => true, 'redirect_url' => $redirect, 'status' => 'approved']);
    } catch (Exception $e) {
        $pdo->rollBack();
        log_process("Erro checkout gratuito: " . $e->getMessage());
        returnJsonError('Erro ao liberar acesso. Tente novamente.', 500);
    }
}
...
try {
    ...
    $stmt_user = $pdo->prepare("SELECT mp_access_token, pushinpay_token, efi_client_id, efi_client_secret, efi_certificate_path, efi_pix_key, efi_payee_code, beehive_secret_key, beehive_public_key, hypercash_secret_key, hypercash_public_key, stripe_secret_key, stripe_publishable_key FROM usuarios WHERE id = ?");
    $stmt_user->execute([$usuario_id]);
    $credentials = $stmt_user->fetch(PDO::FETCH_ASSOC);
    if ($credentials && !isset($credentials['stripe_secret_key'])) {
        $credentials['stripe_secret_key'] = null;
        $credentials['stripe_publishable_key'] = null;
    }
    ...
    if ($gateway_choice === 'efi') {
        ...
        // CRIAR NOTIFICAÇÃO DE PIX GERADO
        if (function_exists('notificar_pix_gerado')) {
            // Buscar ID da venda recém criada
            $stmt_venda = $pdo->prepare("SELECT id FROM vendas WHERE transacao_id = ? LIMIT 1");
            $stmt_venda->execute([$payment_id]);
            $venda_criada = $stmt_venda->fetch(PDO::FETCH_ASSOC);
            if ($venda_criada) {
                notificar_pix_gerado($pdo, $venda_criada['id'], $data['transaction_amount'], $main_product_name);
            }
        }
    ...
    elseif ($gateway_choice === 'pushinpay') {
        ...
        if ($http_code >= 200 && $http_code < 300 && isset($res_data['qr_code_base64'])) {
            ...
            // CRIAR NOTIFICAÇÃO DE PIX GERADO (PushinPay)
            if (function_exists('notificar_pix_gerado')) {
                $stmt_venda = $pdo->prepare("SELECT id FROM vendas WHERE transacao_id = ? LIMIT 1");
                $stmt_venda->execute([$payment_id]);
                $venda_criada = $stmt_venda->fetch(PDO::FETCH_ASSOC);
    ...
    // ==========================================================
    // FLUXO STRIPE (PaymentIntent - confirmação no frontend)
    // ==========================================================
    elseif ($gateway_choice === 'stripe') {
        ...
        if ($amount_cents < 50) {
            throw new Exception("Valor mínimo para Stripe é R$ 0,50.");
        }
        ...
        $pi_result = stripe_create_payment_intent($secret_key, $amount_cents, 'brl', $metadata, 'Compra: ' . $main_product_name);
        
        if (!$pi_result || (isset($pi_result['error']) && $pi_result['error'])) {
            $msg = $pi_result['message'] ?? 'Erro ao criar pagamento Stripe.';
            log_process("Stripe: " . $msg);
        ...
        save_sales($pdo, $data, $main_product_id, $payment_intent_id, 'pending', 'Cartão de Crédito (Stripe)', $checkout_session_uuid, $utm_parameters);
        
        $redirect_url = $redirect_url_after_approval . '?payment_id=' . urlencode($payment_intent_id);
        returnJsonSuccess([
    ...
    // ==========================================================
        ...
        if ($http_code >= 200 && $http_code < 300 && isset($res_data['status'])) {
            ...
            if ($status == 'pending' && $data['payment_method_id'] == 'pix') {
                ...
                // CRIAR NOTIFICAÇÃO DE PIX GERADO (Mercado Pago)
                if (function_exists('notificar_pix_gerado')) {
                    $stmt_venda = $pdo->prepare("SELECT id FROM vendas WHERE transacao_id = ? LIMIT 1");
                    $stmt_venda->execute([$payment_id]);
                    $venda_criada = $stmt_venda->fetch(PDO::FETCH_ASSOC);
