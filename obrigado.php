        ...
        try {
            ...
            if (strpos($body, $loop_start) !== false) {
                 ...
                 foreach ($products as $p) {
                     ...
                     // Limpa tags condicionais - mapeia tipos para tags do template
                     $type_to_tag = [
                         'link' => 'PRODUCT_TYPE_LINK',
                         'pdf' => 'PRODUCT_TYPE_PDF',
                         'area_membros' => 'PRODUCT_TYPE_MEMBER_AREA',
                         'produto_fisico' => 'PRODUCT_TYPE_PHYSICAL_PRODUCT'
                     ];
                     
                     foreach($type_to_tag as $t => $tag) {
                         if ($p['content_type'] == $t) {
                             // Mantém o bloco deste tipo
                             $item = str_replace(["<!-- IF_$tag -->", "<!-- END_IF_$tag -->"], '', $item);
                         } else {
                             // Remove o bloco de outros tipos
...
// NEW: Variables for Memebers Track
    ...
    try {
        ...
        if ($sale_details && $sale_details['email_entrega_enviado'] == 0) {
            ...
            if ($sale_details['status_pagamento'] === 'approved') {
                ...
                $stmt_gateway = $pdo->prepare("SELECT u.mp_access_token, u.pushinpay_token, u.stripe_secret_key, p.gateway FROM produtos p LEFT JOIN usuarios u ON p.usuario_id = u.id WHERE p.id = ?");
                ...
                if ($gateway_info) {
                    ...
                    // Verifica Stripe (transacao_id começa com pi_)
                    if (!empty($gateway_info['stripe_secret_key']) && strpos($payment_id, 'pi_') === 0) {
                        require_once __DIR__ . '/gateways/stripe.php';
                        $stripe_status = stripe_retrieve_payment_intent($gateway_info['stripe_secret_key'], $payment_id);
                        if ($stripe_status && ($stripe_status['status'] === 'succeeded' || $stripe_status['status'] === 'processing')) {
                            $status_aprovado = true;
                            $pdo->prepare("UPDATE vendas SET status_pagamento = 'approved' WHERE transacao_id = ?")->execute([$payment_id]);
                            error_log("Obrigado.php: Status atualizado para 'approved' após verificação Stripe");
                        }
                    }
        ...
        if ($sale_details) {
            ...
            // NEW: Fetch CheckoutPRO Track info
            ...
            if ($starfy_tracking_id_hash) {
                ...
                // Usar caminho fixo para garantir funcionamento correto
                $starfy_track_endpoint = $protocol . $domainName . '/api/api.php';
            }
        }
    } catch (PDOException $e) {
    ...
    <?php if ($sale_details): ?>
        ...
        <script>
            (function() {
                ...
                function sendFlixMembersTrackEvent(eventType, eventData = {}) {
                    const payload = {
                ...
                sendFlixMembersTrackEvent('purchase');
            })();
        </script>
        <?php endif; ?>
        <!-- Fim STARFY TRACK -->
