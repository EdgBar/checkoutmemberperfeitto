...
if (!function_exists('generate_csrf_token')) {
    ...
    function generate_csrf_token() {
        ...
        // Gera novo token APENAS se não existir ou se expirou (1 hora)
        // CORREÇÃO: Removida renovação prematura que causava erro "Token CSRF inválido"
        // O token só é renovado quando realmente expira (após 1 hora)
        $token_age = isset($_SESSION['csrf_token_time']) ? (time() - $_SESSION['csrf_token_time']) : 3601;
        $token_needs_renewal = empty($_SESSION['csrf_token']) || $token_age > 3600;
...
if (!function_exists('verify_csrf_token')) {
    ...
    function verify_csrf_token($token) {
        ...
        if (empty($_SESSION['csrf_token']) || empty($token)) {
            error_log("CSRF Verify: Token vazio ou ausente. Session token: " . (empty($_SESSION['csrf_token']) ? 'VAZIO' : 'OK') . ", Provided token: " . (empty($token) ? 'VAZIO' : 'OK'));
            return false;
        }
...
if (!function_exists('validate_pdf_upload')) {
    ...
    function validate_pdf_upload($file, $upload_dir, $prefix = 'pdf', $max_size_mb = 10) {
        // Alguns ambientes (especialmente Windows/Laragon) podem detectar PDF como application/octet-stream.
        // Mantemos a segurança via validação de conteúdo (%PDF-) em validate_file_content().
